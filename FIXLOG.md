...
- L17 | RESOLVED 2026-08-05 (0ec21a9): tools/NoctisCoverProxy un-ignored (tools/* pattern keeps other tools excluded) and hardening committed. NOT DEPLOYED ANYWHERE — verified 2026-08-05: absent from Noctis.sln, no app-side client (LoonClient speaks a different protocol to the Oracle VM; the proxy's hello/publish/published JSON has no consumer in src/), and no deploy history. Treat as a local side experiment; nothing to redeploy. HMAC auth on /art stays open until it has a real consumer.
...
- L17 | 0ec21a9 | .gitignore, tools/NoctisCoverProxy/* | Proxy now tracked + hardened. Nothing to deploy (see deferred note — it has no consumer and is not the Oracle VM's Loon relay). TEST only if you care: `dotnet run` the proxy, publish non-JPEG bytes → rejected; 17th content_id on one connection → rejected; re-fetch /art past 60 s → 404.
