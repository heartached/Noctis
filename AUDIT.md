...
## High (11 listed, 9 unique — two are cross-lane duplicates)
...
**Scope:** every Critical, High and Medium finding (76 after merging the six cross-lane duplicates), plus the four runtime findings R1–R4 and A10 (low, but in the same engine code). There were no Critical findings. Low findings were out of the requested scope and are listed under "Found but not fixed".

**How each fix was made:** five fix lanes ran at most five at a time, split by file ownership: library/metadata/files, UI performance, security/server/xplat, player/transport, and the audio engine. Each finding got its own fixer agent, which:
- re-verified it at the current HEAD,
- made a minimal root-cause fix,
- added a headless test that **fails without the fix** wherever the logic is testable,
- built with the count-aware zero-new-warnings check,
- ran the targeted tests,
- committed it as `Fix <ID>: …`.

An adversarial reviewer then read the whole lane diff and ran the full suite. A fix-up round (`Fix <ID> (review): …`) handled every medium or high issue the reviewers raised. The audio lanes re-verified audible changes with the silent harness. All lane branches were cherry-picked onto `audit/full-pass`; two conflicts were resolved by hand (keep-alive creation and `ReleasePreparedNext`, both keeping both sides).

**Result:** 77 fixed, 3 partial (P04, A17, A18), 1 disproved (R4). Also fixed on this branch:
- **X16** (`7363b45`), originally built on the auto-update branch.
- **GitHub #70** (`89047f6`).
- **GitHub #97** diagnostics (`379cafa`, `6791eca`).

**Final state of `audit/full-pass`:** build exit 0, **zero new warnings** against the v1.5.4 baseline (count per file+code+message), and a full suite of **3379 passed, 0 failed, 2 skipped** on the final code commit `7363b45`, and 3375/0/2 on two consecutive runs just before X16 was added. Both skips are marked skip in the source.

**Issues the reviews caught and fixed during integration:**
- **X07-a:** "Move to Recycle Bin" now reports whether the user answered **No** to the "permanently delete?" prompt, so the retry ladders stop asking again (`Fix X07-a (review)`).
- **X09:** the YAML Lyricsfile parser now refuses aliases, because anchors could still blow a tiny file up past the line and word caps.
- **X02:** the per-user login throttle table is now bounded, so it can't be used to exhaust memory.
- **Warnings the old check missed:** my first warning check compared warning texts, not counts, so new copies of an existing warning in the same file got through. The check was fixed, and the 11 xUnit1051 warnings it then found were cleared (test-only commits).
- **Racy test:** the U21 Albums test failed on its own because it counted a late filter change as a reload reset. The test now waits for the filter to settle (`41fd975`). The U21 code fix itself was correct: one reset, and the scroll position is kept.

| ID | Sev | Outcome | Commit(s) on `audit/full-pass` | What changed / why skipped | Verification |
|---|---|---|---|---|---|
| A13 | high | fixed | `81243bf6` | This lane had already fixed and committed A13 in f69110c8, before the resume. HEAD (795c45e4) still has the fix: TempoStretchProvider.cs:77-83 and PitchShiftProvider.cs:99-104 use element-store loops instead of Array.Copy into the render buffer, so the NAudio WaveBuffer pun (a byte[] seen as float[]) no longer throws ArrayTypeMismatchException. I grepped the render-chain files (GaplessSpliceCore, GaplessSink, MuteGa… | Re-run at HEAD 795c45e4: warncheck.sh gave build exit=0 and zero new warnings vs the v1.5.4 baseline. I ran dotnet test --no-build with the filter GaplessGapCanaryTests/TempoStretch/PitchShift: 19 passed, 0 failed. The regression test pushes 1.5x speed and +2 semitones, then a return to unity, through a real NAudio WaveBuffer pun. The f69110c8 commit messag… Tests added: 1. |
| A24 | high | fixed | `ad79a9ca` | Confirmed at HEAD 0883966. The TryAdvanceForGapless guard (PlayerViewModel.cs:2967) and the TryAdvanceForAutoMix guard (:2810) had no StopAfterCurrentTrack check, so with songs queued they called AdvanceQueue about 0.5 s (gapless) or up to the fade length (AutoMix) before the real end. The stop branch (:2262-2274) only set State=Stopped and cleared the one-shot flag. The audio played out, OnTrackEnded (:3101) posted… | warncheck.sh: build exit=0, no new warnings compared with the v1.5.4 baseline. The 3 new tests fail without the fix (production change reverted, test build rerun: 3/3 FAIL) and pass with it. Targeted run (StopAfterCurrentHandoffTests + AutoplayQueueTests + AutoMixSeekIntoWindowTests + Gapless*): 40 passed, 0 failed. The gapless tests go through the real pos… Tests added: 3. |
| P01 | high | fixed | `741ac01e` | Confirmed at HEAD 882ad437. In .github/workflows/dotnet.yml:132-142, the "Bundle ffmpeg (macOS)" step ran for every osx-* RID, including osx-arm64. It bundled the evermeet.cx 8.1.2 zip, a thin x86_64 build, even though the comment said it was universal2. AudioConverterService.cs:147-150 returns the bundled copy before it looks on PATH. So without Rosetta 2, an Apple Silicon Mac gets "Bad CPU type in executable" for … | No xunit test was added: the defect is in the CI packaging YAML, not in app code, and nothing in tests/Noctis.Tests checks workflow files. The new lipo -verify_arch checks are the automated gate that fails the build on the wrong architecture. Checks run: I downloaded the pinned arm64 zip to scratchpad/p01fix. Its SHA-256 ef1aa60006c7b77ce170c1608c08d8e4ba1c… |
| P25 | high | fixed | `382c253c` | Re-checked at HEAD e307b68e and the defect is real. At VlcAudioPlayer.cs:434-437 (pre-fix) Linux calls a bare Core.Initialize() and registers no resolver. Nothing in src uses NativeLibrary, SetDllImportResolver or ResolvingUnmanagedDll. I read the metadata of LibVLCSharp 3.10.0 net10.0: its P/Invoke module refs are only libvlc (238), kernel32, libSystem, msvcrt and libc. It has no resolver and no ".so.5" string, onl… | warncheck.sh: build exit=0, no new warnings compared with the v1.5.4 baseline. dotnet test --filter VlcLinuxSonameTests: 5 passed, 0 failed. Without the fix these tests do not compile because the helper is missing. The actual Linux dlopen fallback could not be run on this Windows host, so the Linux runtime load is UNVERIFIED. It needs a check on Ubuntu 22.0… Tests added: 1. |
| R3 | high | fixed | `1ff386f2` | The seek click is real, but the finding's explanation was wrong. At all 4 baseline seeks the declick ramp and the fade-in did engage: run1/tap.wav shows a linear ramp to 0 over 240 frames, then the fade starting. The click sits 128-139 frames into the fade and comes from VLC's own post-seek PCM. The engine input tap (run1/in_tap.raw, frames 250914, 1782471 and 2554431) shows the same step before anything reaches the… | Failing-first: with discardFrames set to 0 (the pre-fix behaviour), the new test fails with 'step of 0.171 at sample 29558'. That is 240 ramp frames plus 139 frames into the fade, the field shape. With the fix it passes. The test also pins PositionMs at 10_020 after the discard and checks that a flush before any audio drops nothing. Targeted tests: GaplessS… Tests added: 1. |
| S04 | high | fixed | `38624578` | Confirmed at HEAD d92ec2d8. In FileOrganizerService.cs:155-178, RemapPlaylistsAsync loaded a fresh copy of playlists.json, remapped its TrackIds and saved it. SidebarViewModel.Playlists (SidebarViewModel.cs:81) kept the old ids. Nothing reloaded it after an organize: LoadPlaylistsAsync is called only at MainWindowViewModel.cs:314/684, PlaylistImportViewModel.cs:199 and SettingsViewModel.cs:1048. Every later save wri… | The new test uses a JSON-backed playlist store (fresh objects on every load, like playlists.json). It loads the sidebar, moves a real temp file through FileOrganizerService.ApplyAsync with FakeLibraryService.RelocateRemap set to {old→new}, then applies the result's remap to the sidebar the way OrganizeFilesViewModel does. It asserts the live playlist is the… Tests added: 1. |
| S07 | high | fixed | `839ed9bd` | Confirmed at HEAD ef122543. Right-clicking a row or tile keeps the Ctrl-selection (MultiSelectHelper.cs:208-209 and :300-301 return early when the left button is not pressed). The views then push the whole selection to the ViewModel even when the clicked item is not in it (LibrarySongsView.axaml.cs:173/193, PlaylistView.axaml.cs:437/550, LibraryAlbumsView.axaml.cs:162, FavoritesView.axaml.cs:95, HomeView.axaml.cs:20… | I stashed the ViewModel changes, rebuilt and ran CtrlSelectionScopeTests: 5 failed and 2 passed. The 5 failures are the outside-selection cases for Songs, Playlist, Albums, Home and Favorites. The 2 inside-selection cases pass both before and after the fix; they guard the selection bar and Ctrl-selection paths. With the fix restored and rebuilt, `dotnet tes… Tests added: 7. |
| S14 | high | fixed | `095eb950` | Confirmed at HEAD. In src/Noctis/ViewModels/MetadataViewModel.cs (lines 2600-2617 before the fix), the multi-select rename called File.Move and set t.FilePath = newPath. It never re-keyed Track.Id, which is MD5 of the path (LibraryService.cs:2867), and it did not suppress the watcher. The only callers of RelocateTracksAsync and SuppressPaths were in FileOrganizerService.cs:126/141. The fix now does what Organize Fil… | The new test turns on multi-select rename and renames two real temp files. It asserts that ILibraryService.RelocateTracksAsync receives both (old, new) path pairs; the FakeLibraryService now records these moves. It fails with the fix reverted ("Collections differ", RelocateTracksAsync never called) and passes with the fix in place. Targeted run: dotnet test… Tests added: 1. |
| X04 | high | fixed | `07d0a5a7` | Confirmed at HEAD. At ServerCertificate.cs:20-22, KeyFlags added EphemeralKeySet on every OS except macOS, and both load paths used those flags (:35 LoadPkcs12FromFile and :75 Create). On Windows, Schannel cannot use an ephemeral key as a server credential, so Kestrel's UseHttps (NoctisServer.cs:72) started normally but every TLS handshake failed. The new test reproduced this before the fix: HttpRequestException 'Th… | The new test runs only on Windows and returns early on other OSes. It starts NoctisServer.StartAsync(0, cert) twice: once with the freshly created certificate (the Create() path) and once with the certificate reloaded from the stored PFX. Each time it sends a real HTTPS GET to rest/ping.view with the fingerprint pinned. Before the fix it FAILED ('The SSL co… Tests added: 1. |
| X07 | high | fixed | `84894227` | Confirmed at HEAD 5f83a0e8. RecycleBin.cs:71 used FOF_ALLOWUNDO / FOF_NOCONFIRMATION / FOF_SILENT / FOF_NOERRORUI and did not set FOF_WANTNUKEWARNING. Line 73 treated a 0 return with no abort as success. The shell recycles only "if possible", and FOF_NOCONFIRMATION answers Yes to its permanent-delete prompt. On a UNC or mapped network share, a USB stick, or an item larger than the bin quota, files were therefore del… | Ran warncheck.sh: build exit=0 with no new warnings. Ran 'dotnet test ... --filter FullyQualifiedName~RecycleBinTests': 11 passed, 0 failed. The new tests exercise the volume classifier that decides whether trashing is allowed. Without the fix they do not compile, because the classifier did not exist. They do not check the SHFileOperation flag change. That … Tests added: 2. |
| A01 | medium | fixed | `08d103cd` | Confirmed at HEAD (caa2935d). In the GaplessSink constructor, SampleRate and Channels were set from the startup default device's mix format (Math.Clamp(mix.SampleRate, 8000, 384000); mix.Channels >= 2 ? 2 : 1). Both are get-only. RebuildLoop reuses the same Provider, and VlcAudioPlayer.cs:699 is the only SetAudioFormat call for the engine players, made at construction. So if the app started on a mono or low-rate end… | warncheck.sh: build exit=0, zero new warnings (ran before and after the revert check). I swapped the helper back to the old rule (clamp rate, 1 channel) as a temporary change: all 8 tests failed. I then restored the fix and rebuilt: GaplessSinkFormatTests + UpmixSampleProviderTests + GaplessSpliceCoreTests passed 36/0. I did not use the silent runtime harne… Tests added: 1. |
| A03 | medium | fixed | `191a17b8` | Checked again at HEAD af4e683d. The finding is real. GaplessSink started its output once with _out.Play() at GaplessSink.cs:132 and never stopped it. Since A07 (1071eddb), Pause() only sets Provider.Parked and never touches WasapiOut. VlcAudioPlayer.Stop (VlcAudioPlayer.cs:4497-4535) calls only EngineClearAll and never touches the sink. The only stop is in Dispose or a rebuild. So the render stream stayed active, ho… | - **Build:** warncheck.sh gave build exit=0 and no new warnings (run twice, the second after reverting a mutation). - **Tests:** dotnet test with filter GaplessSink/GaplessSpliceCoreTests/NullWavePlayerTests: 49/49 passed; GaplessSink filter after the final build: 14/14 passed. - **Mutation check:** making Observe never return Park or Wake (the pre-fix beha… Tests added: 6. |
| A05 | medium | fixed | `b7ae8ee9` | Re-checked at HEAD 8d80aea2 and the defect is still there. In GaplessSink.RebuildLoop, `if (_desiredPlaying) newOut.Play();` runs before the `_out` swap and before `Rebuilt?.Invoke()`. OnGaplessSinkRebuilt (VlcAudioPlayer.cs) then hops to the ThreadPool and does Invalidate, SetLevel and a full Resolve before SetMasterVolume lands. Nothing in the render chain applied gain in between: MuteGateProvider only handled mut… | warncheck.sh: build exit=0 with zero new warnings; I ran it twice, the second time after restoring the source. `dotnet test --no-build --filter "FullyQualifiedName~MuteGateProviderTests/FullyQualifiedName~GaplessSink"`: 24 passed, 0 failed. Negative check: with the hold branch in Read turned off, 3 of the new tests failed (Hold_SilencesFromTheFirstRead, Rel… Tests added: 5. |
| A07 | medium | fixed | `dbfcf7a1` | This was already fixed before this resumed session, in commit 1071eddb on audit/fix-engine. A09 (795c45e4) was committed on top later. The working tree was clean on resume, so I made no new edits. I re-checked the fix at HEAD and it is intact. Cause: GaplessSink.Pause/Resume called WasapiOut.Pause/Play. NAudio's Pause only stops filling the buffer, so the device played its ~100 ms queue and then starved mid-waveform… | Re-run at HEAD 795c45e4 in this session. warncheck.sh: build exit=0, no new warnings against the v1.5.4 baseline. dotnet test with the filter GaplessSpliceCoreTests/GaplessCrossfadeTests: 29 passed, 0 failed. Both Park_* tests pass, as does A09's Park_DuringCrossfade_RampsFromTheMixedFrame. According to the original commit message, the ramp/fade test fails … Tests added: 2. |
| A08 | medium | fixed (by its earlier commit) | `a76f57af` | This was already fixed at HEAD by commit 5ef9f99d ("Fix A08: size the junction declick and fade in whole frames"), which is earlier in this lane. At src/Noctis/Services/GaplessSpliceCore.cs:424-425 both lengths are now whole frames times channels: `_startFadeSamples = SampleRate * Clamp(startFadeMs,0,100) / 1000 * Channels` and `_fadeArmSamples = SampleRate * FadeArmMs / 1000 * Channels`. At 44.1 kHz stereo the ramp… | Ran warncheck.sh on wt-fix-engine at HEAD 795c45e4: build exit=0, no new warnings. Ran `dotnet test --no-build --filter FullyQualifiedName~GaplessSpliceCoreTests`: 21 passed, 0 failed, including the 44.1 kHz stereo channel-parity test. The working tree was clean before and after. Tests added: 1. |
| A09 | medium | fixed | `6e99a0ca` | Resumed run. The fix for A09 was already committed in this lane as 795c45e4 (HEAD) by the interrupted earlier run, and the worktree was clean. I re-checked it rather than redoing it. Cause: GaplessSpliceCore.Read captured _lastFrame from the raw incoming audio before MixFadingTail rescaled it and added the outgoing tail. When Clear() (skip, new play, Stop) or a park dropped the tail mid-blend, the next cut ramp star… | VERIFIED in this session. (1) warncheck.sh on wt-fix-engine: build exit=0, no new warnings listed. (2) At HEAD, dotnet test --filter FullyQualifiedName~GaplessCrossfadeTests: 8/8 passed. (3) With GaplessSpliceCore.cs temporarily reverted to 795c45e4^ and the test project rebuilt, the 3 new tests fail ('cut stepped: 0.27188024 -> -0.5' for Clear and Park, '0… Tests added: 3. |
| A11 | medium | fixed (by A13's commit) | `81243bf6` | Already fixed at HEAD by commit f69110c8 ("Fix A13: copy speed/pitch output into the render buffer with element stores"). The finding pointed at PitchShiftProvider.cs:101, where the code used Array.Copy(_in, ..., buffer, ...). At HEAD, DrainBufferedAtUnity (src/Noctis/Services/PitchShiftProvider.cs:99-103) copies into the render buffer with element stores instead: `for (var s = 0; s < take * _channels; s++) buffer[o… | Read PitchShiftProvider.cs at HEAD 795c45e4 and confirmed the unity drain uses element stores (lines 99-103). Ran dotnet test tests/Noctis.Tests/Noctis.Tests.csproj --filter "FullyQualifiedName~GaplessGapCanaryTests/FullyQualifiedName~PitchShift": build succeeded, 12 passed, 0 failed. This includes SpeedAndPitch_RenderThroughNAudioWaveBufferPun. git status … |
| A12 | medium | fixed | `ee3268cb` | Already fixed in this lane by commit 578543b2 (made before this resume). Cause: PitchShiftProvider set _ended on the first 0-read from its source and never cleared it. The source is the segment ring, which also returns 0 on a mid-track underrun, so one stall while pitch-shifted silenced the rest of the track (at unity pitch too), and the segment never finished. Fix: the _ended field is removed. Fill returns false wh… | Re-run at HEAD 795c45e4: warncheck.sh gave build exit=0 with zero new warnings against the v1.5.4 baseline. `dotnet test --no-build --filter "FullyQualifiedName~PitchShiftProviderTests"` passed 8/8, including the 2 new underrun tests. That the new tests fail without the fix is taken from commit 578543b2's message (1599 and 1 frames before the fix); I did no… Tests added: 2. |
| A16 | medium | fixed | `72c01683` | Confirmed at HEAD: VlcAudioPlayer.ReadReplayGainTags (src/Noctis/Services/VlcAudioPlayer.cs:2142-2176 before the fix) read only the ID3v2, Xiph and Apple tags. AdvancedTagIO.WriteCustomField (src/Noctis.Core/Services/AdvancedTagIO.cs:585-601) and the RG scanner (ReplayGainScannerService.cs:295) write to the APE and ASF tags. So on a WMA file, where only the ASF tag exists, the scanned value was never read back. The … | Before the fix, all 3 new tests failed with 'Expected -7.84 / -4.20 / 1.25, Actual: null'. After the fix, warncheck.sh reported build exit=0 and no new warnings compared with the v1.5.4 baseline. The command dotnet test --no-build --filter "ReplayGainTagReadTests/ReplayGainScannerViewModelTests/AdvancedTagAliasTests" passed 12/12. The runtime harness was no… Tests added: 3. |
| A17 | medium | partial | `54134ae2` | The defect was real at HEAD 7fdb26a4. Three separate checks stopped remote tracks from getting gapless/crossfade: - VlcAudioPlayer.cs:2411: PrepareNext returned on !File.Exists, so a URL was never staged. - The engine splice gate required !isPathless and compared the staged path against Path.GetFullPath(filePath). - PlayerViewModel.cs:2896 and :3014: the early AutoMix and gapless advance also bailed on !File.Exists.… | - New tests fail without the fix: with the interface/fake scaffolding in place but the view-model gate unchanged, the two 'stages streams' tests failed on Assert.True at RemoteStreamTransitionTests.cs:55 and :88 (2 failed, 2 passed). - After the fix, the targeted classes (RemoteStreamTransitionTests, AutoMixSeekIntoWindowTests, MediaServerClientTests, Audio… Tests added: 4. |
| A18 | medium | partial | `4364aea9` | Re-verified at HEAD e452a9c6: PlayInternal sets _currentMediaPath and calls ApplyReplayGain for the incoming track (VlcAudioPlayer.cs ~2771-2776) before the engine splice branch. ReapplyVolume then ramps the single OS session. The outgoing segment keeps rendering from the ring, and during BeginCrossfade it keeps rendering for the whole fade. PlayerViewModel.OnCurrentTrackChanged (PlayerViewModel.cs:1721) re-applies … | - warncheck.sh: build exit=0 and zero new warnings. - dotnet test with filter GaplessReplayGainTests/GaplessSpliceCoreTests/GaplessCrossfadeTests/GaplessGapCanaryTests/ReplayGain: 43 passed, 0 failed. - With the per-sample gain multiply temporarily removed, 3 of the 4 new tests failed (seam, crossfade, slew). The file was then restored. - The VlcAudioPlayer… Tests added: 4. |
| A19 | medium | fixed | `67af92da` | An earlier run of this lane already fixed and committed A19 (5364ba4e, "Fix A19: honor a pause in the engine tail and while a track is opening"). The later commits (7fa6e56d..b79c2aa0: A25, R1, A27) leave it in place. At HEAD, src/Noctis/Services/VlcAudioPlayer.cs:4438 gates the pause on intent (`_currentMedia != null && !_isPaused`) instead of `_player.IsPlaying`. :4452 uses `_player.SetPause(true)` and still pause… | Checked again at HEAD b79c2aa0. warncheck.sh: build exit=0, no new warnings against the v1.5.4 baseline. dotnet test with the filter Vlc/ShutdownPause: 51 passed, 0 failed. I did not add a headless test: the gate needs a live LibVLC MediaPlayer, and the tests never construct VlcAudioPlayer. The original commit recorded a silent-harness check. Before the fix… |
| A21 | medium | fixed | `6f0d753c` | Re-verified at HEAD 0e3c54b3. In the end watchdog (VlcAudioPlayer.cs, formerly lines 4985-4989), `drained` was true whenever the sink's active segment came from the other slot: `(wdSeg.Source is int s && s != EngineSlotOf(_player))`. A short next track finishes decoding while it is still staged, and its EndReached is logged as IgnoredInactive. After the bookkeeping-only splice, `_player` is that Ended player while t… | Build with warncheck.sh: exit=0, no new warnings. Targeted tests (filter GaplessEndWatchdogTests/GaplessSpliceCoreTests/GaplessReplayGainTests): 28/28 passed, 3 of them new. The first new test builds the exact state in which the old expression returned drained=true (ActiveSegment is the slot-0 tail, the player is slot 1) and asserts not-drained. The helper … Tests added: 3. |
| A22 | medium | fixed | `54767bb7` | Confirmed at HEAD. ReleasePreparedNext (VlcAudioPlayer.cs, formerly around line 5273) always abandoned `_engineSegments[EngineSlotOf(_standbyPlayer)]`, stopped the standby and set its volume to 0. After an engine splice (lines 2908-2918), `_standbyPlayer` is the outgoing player and that segment is its audible tail: the crossfade `_fading` tail from BeginCrossfade, or the EOS ring tail. CancelPreparedNext is called o… | Build: warncheck.sh showed build exit=0 and no new warnings (run after both source changes). Tests: PauseResume_MidCrossfade_FadesTheKeptTailBackIn failed before the provider change ("resume stepped out of silence to 0.498") and passes after. `dotnet test --filter FullyQualifiedName~Gapless` passed 67/0. The release-decision tests call the new helper, so wi… Tests added: 7. |
| A23 | medium | fixed | `a4ea9605` | Confirmed at HEAD e22427ec. WasapiGainOutput.Flush (old :333-337) only called _buffer.ClearBuffer(). With ReadFully=true (old :240) the queue zero-filled, and GainSampleProvider.Read (old :438-502) only slewed the volume and park gains. So every seek/stop flush and every underrun stepped from the last emitted frame to 0, and post-cut PCM restarted at arbitrary amplitude. The seek worker leaves the callback-sink path… | The underrun test was run before the fix and failed: 'Expected: 1764, Actual: 441' (short read, no pad or declick). The cut test uses the new Cut API, so it cannot compile without the fix. After the fix, warncheck.sh gave 'build exit=0' and listed no new warnings. 'dotnet test --no-build --filter FullyQualifiedName~WasapiGain' passed 5/5 (the 3 existing par… Tests added: 2. |
| A25 | medium | fixed | `eca9d612` | An earlier run in this lane had already fixed this and committed it as 76e3028a, but never reported it. I checked it again at HEAD b79c2aa0 and changed no code. Cause: PlayInternal's header parse waited on _skipCts. Pause (4226), Seek (4364) and CancelPreparedNext also cancel _skipCts, and CancelPreparedNext runs on every queue edit through CancelAutoMixTransition. If one of them landed during the parse, PlayInterna… | - Re-checked at HEAD b79c2aa0 with a clean working tree. - warncheck.sh: build exit=0, zero new warnings against the v1.5.4 baseline. - dotnet test --filter FullyQualifiedName~Vlc: 48 passed, 0 failed. - No headless test was added. The race needs a live LibVLC Media.Parse, and no test in tests/ constructs VlcAudioPlayer (grep for "new VlcAudioPlayer" finds … |
| A26 | medium | fixed | `170e0b08` | Confirmed at HEAD fff8406b. RestoreQueueStateAsync (PlayerViewModel.cs:1581-1586) leaves the player Stopped with no media loaded and sets _resumePositionMs. SeekToPosition (the old :630), EndSeek's debounce timer (the old :1945) and Previous's restart (:534) all called _audioPlayer.Seek. VlcAudioPlayer.Seek:4531 drops that seek when _currentMedia is null; the baseline run2 log shows 'Seek.Dropped reason=noMedia' fol… | warncheck.sh: build exit=0, zero new warnings. The 4 new stopped-state tests fail with PlayerViewModel.cs reverted to HEAD (the Playing control test passes) and pass with the fix. Targeted run with the filter StoppedSeekResumeTests/QueueRestore/StopAfterCurrent/Seek/PlayerViewModel/AutoplayQueueTests/Gapless: 77 passed, 0 failed. Silent harness on the fixed… Tests added: 5. |
| A27 | medium | fixed | `279f7a04` | This was already fixed and committed before the resume, as b79c2aa0 (HEAD of audit/fix-player). The cause: the queue restore set PlayerViewModel.IsMuted from the saved state (PlayerViewModel.cs:1561), but the change handler only refreshed the signal-path badge. The audio player's mute was written only by ToggleMute and UnmuteForAdjust. So after quitting while muted, the app showed Muted, played audibly, and the firs… | Re-run at HEAD b79c2aa0: warncheck.sh gave build exit=0 and no new warnings against the v1.5.4 baseline. 'dotnet test --no-build --filter FullyQualifiedName~MuteRestoreTests' gave Passed 2/0 failed. The restore test's Assert.True(audio.IsMuted) would fail without the fix: FakeAudioPlayer.IsMuted is a plain auto-property (PlayerTestFakes.cs:28), and before t… Tests added: 2. |
| A28 | medium | fixed | `9a60718c` | Confirmed at HEAD b79c2aa0. In src/Noctis/ViewModels/PlayerViewModel.cs, OnLibraryUpdated called AdvanceQueue() with the default reason, Natural, whenever the current track was gone and UpNext was not empty. AdvanceQueueCore then called PlayTrack, which always sets State=Playing and calls _audioPlayer.Play. So a paused player started the next song by itself, and so did a restored session (State=Stopped) whose file a… | warncheck.sh: build exit=0, no new warnings (run before and after the fail check). With the PlayerViewModel.cs change reverted, all 4 new RemovedCurrentTrackTests fail (Failed: 4). With the fix, the targeted filter passes 88/88: RemovedCurrentTrackTests, QueueGitHub82_84_85Tests, DiscordBatch20260921Tests, QueueRestoreIssue91And92Tests, StopAfterCurrent*, E… Tests added: 4. |
| A29 | medium | fixed | `e2d2faac` | Confirmed at HEAD d6f8e852. The Repeat All wrap (PlayerViewModel.cs, AdvanceQueueCore, about line 2346) replayed _repeatCycleTracks unchanged. Only ReplaceQueueAndPlay (line 1189) and the restore (line 1569) write that list. The wrap cleared _originalQueue and never checked IsShuffleEnabled. No queue edit touched the snapshot, and StopAndClear never cleared it. Root cause: the cycle was a start-time snapshot that no… | With the PlayerViewModel.cs change reverted, PlayerQueueTests gave Failed 5, Passed 4: all 5 new tests failed. With the fix applied it gave 9/9 passed. Other targeted classes all passed: AutoplayQueueTests 7/7, ExplicitContentFilterTests 13/13, QueueRestoreIssue91And92Tests 4/4, and HomeRecentRowsFromPlayLog, QueueExplicitInstance, QueueGitHub82_84_85, Play… Tests added: 5. |
| A30 | medium | fixed | `013c77a1` | Confirmed at HEAD 5921039e: PlayerViewModel.cs:2740-2748 has a 9s stale-position guard (TrackStartStalePositionGuardMs=9000, line 353). PlayTrack re-arms it at line 2080 and every seek re-arms it at lines 634 and 1995. The guard accepted a tick only if it was at most target + wall-clock elapsed + 4s. Media time runs at the playback rate: on the VLC path through its rate, and on the engine path through the TempoStret… | warncheck.sh: build exit=0, no new warnings. With the source line temporarily reverted, DoubleSpeed_TickSixSecondsAfterStart_IsApplied failed (1 failed / 2 passed). With the fix restored and rebuilt, all 3 new tests passed. The related position/seek test classes also passed (StopAfterCurrentHandoffTests, AutoMixSeekIntoWindowTests, StoppedSeekResumeTests, L… Tests added: 3. |
| A31 | medium | fixed | `4f96c25f` | Confirmed at HEAD fbbed680. In OnPlaybackError (PlayerViewModel.cs, about line 3239 before the fix), the 5-error cascade called StopAndClear("errorCascade"), and an error with UpNext empty called StopAndClear("errorNoNext"). StopAndClear (line 1279) sets CurrentTrack to null and clears UpNext, History, _precedingInQueue, _originalQueue and _repeatCycleTracks. The next SaveQueueStateInBackground or the shutdown SaveQ… | All 3 new tests FAILED against the unfixed PlayerViewModel.cs, checked by reverting the source only and rebuilding: Failed 3, Passed 0. They pass with the fix. warncheck.sh reports build exit=0 and no new warnings. Targeted run: dotnet test --no-build with a filter covering PlaybackErrorStopTests, RemovedCurrentTrackTests, StoppedSeekResumeTests, AutoplayQu… Tests added: 3. |
| A32 | medium | fixed | `8734748c` | The finding was still real at HEAD e245566f. In MainWindowViewModel.ShutdownAsync, three slow steps ran first: the server stop (up to 2 s, line 957), the scan pause (up to 5 s, line 964) and the scrobble flush (up to 3 s, line 971). Only after them came SetVolume and the settings save (976-977), the play-history flush (979), the queue save (990) and the library flush (994). App.axaml.cs:225 gives up on ShutdownAsync… | - Build (warncheck.sh): exit=0, no new warnings. - Targeted tests: ShutdownSaveOrderTests, SettingsViewModelPersistenceTests and ShutdownPauseTests, 20/20 passed. - Old-order check: I temporarily put back the HEAD version of MainWindowViewModel.cs. UserStateSaves_RunBeforeTheSlowSteps failed against it, then I restored the fixed file. - ShutdownAsync needs … Tests added: 3. |
| A34 | medium | fixed | `740cd386` | Confirmed at HEAD 82a813c7. In MprisService.cs:620-636, the Shuffle and LoopStatus writes set _player.IsShuffleEnabled and RepeatMode directly. IsShuffleEnabled is a plain [ObservableProperty] (PlayerViewModel.cs:106) with no change hook. Only ToggleShuffle (PlayerViewModel.cs:739-805) reorders UpNext and saves _originalQueue. So a Shuffle set over MPRIS turned the indicator on but left the queue in album order, and… | warncheck.sh: build exit=0, no new warnings. I temporarily put back the old direct `player.IsShuffleEnabled = shuffle` and the shuffle test failed as expected (1 failed, 7 passed). After restoring the fix, the filter MprisPropertySetTests/PlayerQueueTests/LocalApiServerTests gave 56 passed and 0 failed. The D-Bus PropertiesChanged emission is Linux-only and… Tests added: 3. |
| P04 | medium | partial | `3fe5c141` | The finding is real at HEAD. ComputeFileId still lowercases the path (LibraryService.cs ~2868). The scan fast path (~298-306) and the import fast path (~1149-1153) re-added the known track with its old FilePath whenever size and mtime matched, so a case-only rename kept a path that no longer exists. In WatchDebouncer, the indexer set on the OrdinalIgnoreCase dictionary kept the OLD key, so Drain imported the old spe… | I stashed only the two src files, rebuilt and ran the targeted classes: both new tests FAIL (2 failed / 16 passed). With the fix restored and rebuilt: 'dotnet test ... --filter "FullyQualifiedName~WatchDebouncerTests/FullyQualifiedName~FileSystemSourceScanTests"' gives Passed 18, Failed 0. The scan test uses the existing StreamOnlySource fake with a name tr… Tests added: 2. |
| P05 | medium | fixed | `1524b280` | Confirmed at HEAD. LibraryWatcherService.Refresh() (src/Noctis.Core/Services/LibraryWatcherService.cs:47-91) set EnableRaisingEvents=true on a recursive FileSystemWatcher on the calling thread. That call is the Linux inotify tree walk. Its callers run on the UI thread: MainWindowViewModel.cs:493 (MusicFoldersChanged), MainWindowViewModel.cs:716 (InitializeAsync at launch) and SettingsViewModel.cs:3570 (the Watch Fol… | The new test uses a settings accessor that blocks until the test releases it, then checks whether Refresh() returns while the rebuild is still running. I ran it against the unfixed source by temporarily restoring the original file. It FAILED with "Refresh blocked the caller until the watcher rebuild finished" (5 s timeout). With the fix it passes: dotnet te… Tests added: 1. |
| P06 | medium | fixed | `96a64c53` | The bug is still present at HEAD (3024c08f), in a slightly different form. The P05 fix moved the rebuild onto the thread pool (LibraryWatcherService.cs:47-56, RebuildWatchers holds lock(_gate) at :60). But OnError (:266-273) still calls Refresh() for every error. On Linux, a folder that can't be watched (ENOSPC watch limit, or EACCES on a subfolder) raises Error synchronously from inside EnableRaisingEvents (:93). T… | Neither test can make a real Linux inotify limit happen on Windows. Instead, they use reflection to take the private _gate and call the private OnError while holding it, which is the same thread and lock state as a synchronous setup error. Before the fix, ErrorRaisedDuringWatcherSetup_DoesNotQueueAnotherRebuild FAILED (expected 0 rebuilds, got 3: one queued… Tests added: 2. |
| P08 | medium | fixed | `53405186` | Confirmed at HEAD 01ebd181. In MetadataService.cs, TryGetFolderArtFile (around lines 425-449) used File.Exists(Path.Combine(directory, name + ext)) with the all-lower-case FolderArtNames and FolderArtExtensions (lines 402-403). That check is case-sensitive on Linux, so Folder.jpg, Cover.jpg and cover.JPG were never found. IsFolderArtCandidate (lines 410-417) already ignored case. MusicVideoLocator.cs lines 24-37 had… | I checked the new tests against the old code by reverting the two source files and running them. Both failed. On Windows the old probe matched but returned the probed lower-case name, for example 'folder.jpg' where 'Folder.JPG' was expected, so the tests fail on Windows as well as on Linux, where the old code finds nothing. With the fix restored, warncheck.… Tests added: 2. |
| P09 | medium | fixed | `0611e86b` | I confirmed the defect at HEAD in src/Noctis.Server/Program.cs:158-174, and it is worse than the finding says. 1. **SIGTERM.** The only SIGTERM path was the ProcessExit hook at line 160. The web host (WebApplication.CreateSlimBuilder in NoctisServer.cs:63) uses ConsoleLifetime, which registers its own SIGTERM handler that cancels the signal. The .NET 10 Microsoft.Extensions.Hosting.dll contains HandlePosixSignal and… | No test was added. The fix is in noctis-server's Main/ServeAsync, and tests/Noctis.Tests does not reference the Noctis.Server exe project: its csproj references Noctis, Core, Core.Server, UI, Mobile and plugins only. Covering it would mean adding that reference and pulling the inline shutdown sequence out into its own method, which is a refactor. SIGTERM al… |
| P12 | medium | fixed | `2263ca51` | Confirmed at HEAD a366cdfb. ScrubAppImageEnvironment had only two callers: TryShowInLinuxFileManager (PlatformHelper.cs:142) and TryRunHostTool (:269). Five launch sites skipped it: OpenUrl shell-execute (:169-173), OpenFileWith non-macOS branch (:78-83), ReadPortalColorScheme (:425-437), ReadGSettings (:485-494) and RecycleBin.RunProcess (RecycleBin.cs:180-190, used by gio trash and osascript). So xdg-open and the … | warncheck.sh: build exit=0. 'dotnet test --no-build --filter "FullyQualifiedName~PlatformHelperAppImageTests/FullyQualifiedName~RecycleBin"' ran 9 tests: 9 passed, 0 failed. The scrub logic itself is already covered by PlatformHelperAppImageTests. I added no new test that fails without the fix. Each changed site builds the ProcessStartInfo and starts a real… |
| P15 | medium | fixed | `a8da4c78` | The defect was confirmed at HEAD. Program.cs:36-38 reads the files to open only from argv, and nothing under src/ subscribed to IActivatableLifetime or FileActivatedEventArgs. Avalonia 12.1.2 routes macOS openFiles: through AvaloniaNativeApplicationPlatform.FilesOpened to lifetime.OnActivated(FileActivatedEventArgs) and does not buffer it. Application.TryGetFeature(typeof(IActivatableLifetime)) returns the MacOSActi… | - warncheck.sh: build exit=0, zero new warnings. - `dotnet test --no-build --filter FullyQualifiedName~FileActivationTests`: 4 passed, 0 failed. - The workflow YAML parses (python yaml), and the Info.plist the heredoc generates parses as XML with CFBundleDocumentTypes present. The tests fake IActivatableLifetime and IStorageItem with DispatchProxy, because … Tests added: 4. |
| P16 | medium | fixed | `96fbf0fa` | Confirmed at HEAD: in src/Noctis/Program.cs, the catch block (lines 128-145) sent startup failures on macOS and Linux only to Console.Error. A launch from Finder, the Dock or a login item has no terminal to show stderr. So the libvlc install guidance built by VlcAudioPlayer.BuildLibVlcMissingMessage (VlcAudioPlayer.cs:5613, thrown at :451) was never shown, and because every launch failed the same way, the crash jour… | warncheck.sh: build exit=0, no new warnings compared with the v1.5.4 baseline. dotnet test --no-build --filter FullyQualifiedName~StartupErrorAlertTests: 3 passed, 0 failed. As a check, I removed the ShowStartupErrorAlert call from Program.cs and ran the tests again: Program_NonWindowsCatch_ShowsTheAlert failed. I then restored the file. The other two tests… Tests added: 3. |
| P17 | medium | fixed | `32c13b9c` | Confirmed at HEAD 49d4b5bd. In LyricsWriter.cs:149-154, WriteSidecar logged a failed TrashFile but still set replaced=true and overwrote the user's .lrc/.elrc. LyricsStudioViewModel.cs:682 then showed "old .lrc moved to the recycle bin". The Save at :672 also called SaveDetailed synchronously on the UI thread. Fix: a foreign sidecar is overwritten only once it is actually gone, using the same `TrashFile // !File.Exi… | To check the new tests, I temporarily put back the old logic: overwrite after a failed trash, no kept flag, and the synchronous SaveDetailed call. All 3 new tests failed (Failed: 3, Passed: 14). I then restored the fix and rebuilt. Running `dotnet test --no-build --filter "FullyQualifiedName~LyricsWriterTests/FullyQualifiedName~LyricsStudioComposeTests/Full… Tests added: 3. |
| P24 | medium | fixed | `040417b9` | Confirmed at HEAD f3046c0a. TryFindMacLibVlcPath (VlcAudioPlayer.cs:5606-5619) returned /Applications/VLC.app/Contents/MacOS/lib before the bundled Contents/MacOS/libvlc/lib whenever libvlc.dylib merely existed there. Core.Initialize(macLibPath) ran once (:432), and :439-445 rethrew any failure as InvalidOperationException. In LibVLCSharp 3.x (Core.cs LoadLibVLC/LoadNativeLibrary, Core.Desktop.cs Initialize->EnsureV… | warncheck.sh reported build exit=0 and no new warnings compared with the v1.5.4 baseline. `dotnet test --no-build --filter VlcMacLibPathTests/VlcDemuxPolicyTests/VlcReadAheadTests` passed 21/21. I first extracted the helper with the ORIGINAL order and ran the new test: it failed 1/5 (bundled vs VLC.app), then passed after the swap. I did not use the silent … Tests added: 1. |
| P28 | medium | fixed | `7aba00a4` | Confirmed at HEAD aa9b22a9. In MainWindow.axaml.cs, RemapWindowsAfterResume called Hide() and then Show() on every visible window, with MainWindow first. In Avalonia 12.1.2, Window.Hide() (Window.cs:867-893) hides every owned child, sets Owner = null and disposes _modalSubscription. That disposable (Window.cs:1105-1111) activates the owner and completes ShowDialog with the default result, as if the user cancelled. S… | warncheck.sh: build exit=0, no new warnings compared with the v1.5.4 baseline. 'dotnet test ... --filter FullyQualifiedName~LinuxResumeWatcherTests': 11 passed, 0 failed. Negative check: with the dialog guard temporarily disabled (if (false && ...)), the dialog test FAILED at LinuxResumeWatcherTests.cs:51 on Assert.False(result.IsCompleted). So in real head… Tests added: 2. |
| P29 | medium | fixed | `9f521c6e` | Re-verified at HEAD d9908443. Three places checked only `_trayIcon != null`: the start-minimized-at-login hide (MainWindow.axaml.cs:524), minimize-to-tray (:822) and close-to-tray (:1094). App.axaml.cs:171-175 also turns off ShowInTaskbar for login launches. Avalonia's Linux TrayIcon (a StatusNotifierItem) is created without error even when nothing owns org.kde.StatusNotifierWatcher, as on stock GNOME. So Noctis cou… | warncheck.sh: build exit=0, no new warnings. 'dotnet test --no-build --filter FullyQualifiedName~LinuxTrayHostTests/FullyQualifiedName~LinuxResumeWatcherTests': 19/19 passed. To check the tests catch the bug, I temporarily put back the old logic (IsTrayUsable => trayIconCreated; no taskbar restore when there is no tray). Two tests failed: the GNOME case (cr… Tests added: 4. |
| P30 | medium | fixed | `e279f32d` | Confirmed at HEAD 78bd1124. Three paths hide the window with Hide(): close-to-tray (MainWindow.axaml.cs OnMainWindowClosing), minimize-to-tray (the WindowState observer) and start-minimized (SettleStartMinimized). ShowFromTray was only reached from the tray menu or click, the single-instance pipe and file activation. Nothing handled ActivationKind.Reopen: the only Activated subscriber was FileActivation.Subscribe, w… | Before the fix the new tests failed to compile (CS0117: no SubscribeReopen). warncheck.sh: build exit=0, zero new warnings. dotnet test --filter FileActivationTests: 6 passed, 0 failed. The MainWindow wiring (Post ShowFromTray, mini-player gate) is not covered by a headless test: no test constructs MainWindow, and the real IActivatableLifetime only exists u… Tests added: 3. |
| R1 | medium | fixed (by its earlier commit) | `1d8089cd` | R1 was already fixed on this branch by commit 8a4b3ff5 ("Fix R1: stop treating a user pause as an input stall"), which is intact at HEAD b79c2aa0. The only commit after it (A27) touches only PlayerViewModel.cs and MuteRestoreTests.cs. Evidence at HEAD: VLC's engine pause/resume callbacks now record the pause (VlcAudioPlayer.cs:703-704 -> EnginePause/EngineResume at :4121-4135). On resume, the expected pts is carried… | Re-checked at HEAD in this run. The working tree was clean, and the fix's lines are present at the file:line locations above. `dotnet test tests/Noctis.Tests/Noctis.Tests.csproj --no-build --filter "FullyQualifiedName~VlcReadAheadTests"` passed 15/15, run against the Debug build from 21:51, which is newer than the HEAD commit. Runtime evidence from the sile… Tests added: 2. |
| R2 | medium | fixed | `7d2b12e4` | Confirmed at HEAD (795c45e4). MemoryTrim.cs:61 always ran GC.Collect(2, Forced, blocking: true, compacting: true). Trims are requested after the startup scan, after every LibraryService scan (rescan and folder watcher, LibraryService.cs:618), and after library background init. These often land while music is playing. A blocking compacting gen2 pauses the managed NAudio render thread for as long as the GC runs. Basel… | warncheck.sh: build exit=0, no new warnings (run before the mutation check and again after restoring the fix). dotnet test --filter MemoryTrimTests: 6/6 passed. Mutation check: with the predicate ignored (blocking = true), the new test fails with Assert.DoesNotContain "Item found in collection". The fix was then restored, rebuilt and passed again. Silent ha… Tests added: 1. |
| R4 | medium | not a defect (disproved at runtime) | — | This is not a defect at HEAD. The uneven block RMS at 20.99 s in run1 comes from the signal itself, not from a channel mix-up. The seek cut happened when R was at its peak (0.2496, amplitude 0.25) and L was near a zero crossing (0.0896 of 0.30). The declick ramp takes each channel from its own last value down to zero over 240 frames, so R carries more energy in that 10 ms block. Sample evidence from rt/run1/tap.wav:… | - Ran 'dotnet test tests/Noctis.Tests/Noctis.Tests.csproj --no-build --filter FullyQualifiedName~GaplessSpliceCoreTests' at HEAD 795c45e4. The test DLL was built at 18:47, the same time as the HEAD commit, and the worktree was clean. Result: 21 passed, 0 failed. This includes the A08 channel-alignment test. - Did the tap analysis on run1/tap.wav described i… |
| S13 | medium | fixed | `d33b1281` | Confirmed at HEAD 8550a51f. MetadataViewModel.cs:2516 (new cover) and :2539 (remove cover) called _metadata.WriteAlbumArt inside try/catch and ignored its bool. WriteAlbumArt goes through SaveTagsAtomically (MetadataService.cs:586-594), which catches the error and returns false. The failure was lost, so the dialog closed as saved. ClearOwnTrackArtwork (:2235) then gave the failed track the new cover's fingerprint an… | I added a FailArtPaths set to FakeMetadataService, so WriteAlbumArt returns false for those paths the way SaveTagsAtomically does. Before the fix, both new tests failed: CloseRequested fired, Assert.False(closed) failed, and the result was 2 failed / 30 passed. After the fix, 'dotnet test tests/Noctis.Tests/Noctis.Tests.csproj --no-build --filter "FullyQual… Tests added: 2. |
| S15 | medium | fixed | `40202519` | Confirmed at HEAD 66d53ae2. PlaylistViewModel.RemoveTrack (PlaylistViewModel.cs:666) had no IsSmartPlaylist guard, unlike MoveTrack (:692). PlaylistView.BindContextMenuToTrack (PlaylistView.axaml.cs:404) wired "Remove from Playlist" for every playlist, smart ones included. Smart playlist contents come from SmartPlaylistEvaluator.Evaluate (PlaylistViewModel.cs:357-358), so a removed row came back on the next reload. … | The new headless test (AvaloniaFact) builds a smart playlist with the rule Rating > 3 over 2 tracks, then runs RemoveTrackCommand. It checks that Tracks and TrackCount are unchanged and that SavePlaylistsAsync was not called. With the guard commented out the test failed (1 failed, at SmartPlaylistRemoveTests.cs:71). After restoring the guard and rebuilding,… Tests added: 1. |
| S19 | medium | fixed | `9a0bf08c` | Confirmed at HEAD fbf082fa. ConfirmResetLibrary (SettingsViewModel.cs, formerly ~6124-6345) set `_settings = defaultSettings` but only reset a hand-picked list of properties. Several fields that SyncToSettings writes back were never reset: AccentFollowsArtwork (:2669), ShortcutService.SaveTo (:2642), _lyricsBackgroundOverrides, LyricsBackgroundPausesWithPlayback and MusicVideos* (:2746-2749), NoctisServer* (:2762-27… | The new test seeds a settings.json with non-default values for the affected fields, then runs LoadAsync, ResetSettingsToDefaults and SaveAsync. It then uses reflection to compare every AppSettings property with the first save of a fresh install on an empty temp data dir. It also asserts the shortcut is back to default, the clip override is gone, and FakeAud… Tests added: 1. |
| S22 | medium | fixed | `9fcfbb7e` | Confirmed at HEAD 9cb6e0ea. SidebarViewModel.cs:777 passed the unfiltered PlaylistItems list, smart playlists included, to AddToPlaylistDialogViewModel, which did no filtering (AddToPlaylistDialogViewModel.cs:33-36). The selected id then went to AddTracksToPlaylist (SidebarViewModel.cs:641-644), which had no IsSmartPlaylist guard, unlike OpenAddSongsAsync (:707) and sidebar drop (SidebarView.axaml.cs:154). Fix: the … | I reverted the two src edits, rebuilt and ran SmartPlaylistAddTests: both tests failed (Collections differ; Collection was not empty). I then re-applied the fix, rebuilt and ran SmartPlaylistAddTests, SmartPlaylistRemoveTests and SidebarFolderToggleTests: 8 passed, 0 failed. warncheck.sh reported build exit=0. Tests added: 2. |
| S24 | medium | fixed | `30448c8b` | Confirmed at HEAD. src/Noctis/Views/LrcEditorDialog.axaml.cs:20 used a plain `KeyDown += OnDialogKeyDown`, which is a bubbling handler, and none of the dialog's buttons are Focusable=False (LrcEditorDialog.axaml:94-97, 128-178). Once a button such as Play/Pause has focus, it handles Space itself, so the stamp handler never runs and Space clicks that button. The new test reproduced this: with Play/Pause focused, the … | The headless test builds the real LrcEditorDialog around an LrcEditorViewModel with a player that is playing at 7 s. It focuses the Play/Pause button, presses and releases Space, then checks three things: Lines[0].Timestamp is 7 s, SelectedIndex has moved to 1, and the player is still Playing. With the source change reverted it FAILED ("Expected: 00:00:07, … Tests added: 1. |
| S27 | medium | fixed | `503aa100` | Confirmed at HEAD 890e7213. MainWindow.axaml.cs:744 adds OnQueueKeyDown as a tunnel handler in the constructor. Helpers/WindowKeyForwarder.cs:39-40 adds each page's forwarder to the same TopLevel later, when the page attaches. Avalonia calls one element's tunnel handlers newest first, so the page ran first. It took Ctrl+A and Escape and set Handled, so OnQueueKeyDown (MainWindow.axaml.cs:1830, no handledEventsToo) n… | With the gate line commented out, 3 of the 5 new headless tests failed (the page got Ctrl+A/Escape: Expected 0, Actual 1). The ordering test (PlainWindow_...) and the sidebar-focus test passed. With the gate restored, warncheck.sh showed build exit=0 and no new warnings. 'dotnet test --no-build --filter PageKeyOverlayRoutingTests/GlobalShortcutRoutingTests'… Tests added: 5. |
| S28 | medium | fixed | `263f337d` | Confirmed at HEAD 13007267. MainWindow.axaml.cs:1579 (OnGlobalShortcutKeyDown, a window tunnel handler) let a key through to a TextBox only when it had no modifiers. The defaults at Shortcuts.cs:63-66 are Ctrl/Cmd+Right/Left/Up/Down, so pressing Ctrl+Left in the search box, playlist rename, Lyrics Studio editor or a Settings field ran Previous/Next/Volume and set Handled before the TextBox saw the key. Root cause: t… | warncheck.sh: build exit=0, no new warnings listed. The targeted tests (GlobalShortcutRoutingTests, ShortcutDefaultsTests, ShortcutServiceTests) passed 28/0. To show the new tests catch the bug, I temporarily changed the helper so Ctrl+arrows no longer counted as text-box keys. 3 tests then failed: both cases of the ShortcutDefaults theory, and the headless… Tests added: 2. |
| S30 | medium | fixed | `4967b7df` | Confirmed at HEAD: the old PlaylistView.axaml.cs:77-93 OnDataContextChanged reset the menu and VM subscriptions but never cleared _selectedTracks (:24). The selection was only cleared in OnDetachedFromVisualTree (~:775), and that does not run when the DataTemplate recycles the view for a direct A to B playlist swap. The stale set was then re-applied to rows through SyncContainerVisual in ContainerPrepared (:462) and… | New headless test: mount PlaylistView on playlist A (Shared, Only A), press Ctrl+A, then swap DataContext to playlist B (Shared, Only B) on the same view instance. It asserts that A's SelectedCount is 0 and its CtrlSelectedTracks is empty, that no realized row in B has ctrl-selected, and that right-clicking a B row leaves B's CtrlSelectedTracks empty. With … Tests added: 1. |
| U03 | medium | fixed | `bef24cfb` | Confirmed at HEAD d92ec2d8. QueueRowSelection.cs:150-157: Remap copied the whole selection to a list and re-inserted every entry on each CollectionChanged event. PlayerViewModel.cs:1309-1310 (RemoveManyFromQueue) and 1336-1339 (MoveBlockInQueue) remove one row per event, from the highest index down. MainWindow.axaml.cs:517-524 calls Apply on every event and posts a renumber/sync for each one. After Ctrl+A this comes… | - Wrote the tests first and ran them on the unfixed code. Selection_BlockRemoveOfAHugeSelection (5,000 rows, SelectAll, RemoveAt from high to low as RemoveManyFromQueue does, measured with GC.GetAllocatedBytesForCurrentThread) failed with "allocated 861 MB" against the < 64 MB bound. - Selection_RemapsOnlyTheRowsPastAChange (remove, insert and move below, a… Tests added: 2. |
| U05 | medium | fixed | `7c5c510f` | Confirmed at HEAD 0fe616aa. AlbumDetailViewModel.cs:213 posted RefreshFromLibrary on every LibraryUpdated without checking IsPublishingPartial. When the album was missing from a partial snapshot, :341-344 raised BackRequested. LibraryService.cs:222-241 publishes partial snapshots while _publishingPartial is true (set at :249, cleared at :383, and every exit path raises an authoritative LibraryUpdated afterwards). Li… | - **Without the fix:** I reverted the two source files, rebuilt, and ran both new tests. Both failed. The album test got 1 BackRequested where 0 was expected. The Folders test got SelectedNode null where "C:\Music\B" was expected. - **With the fix restored:** 'dotnet test --no-build --filter FullyQualifiedName~PartialPublishPageTests' passed 2/2. - **Relate… Tests added: 2. |
| U06 | medium | fixed | `e007021c` | Confirmed at HEAD 87110a40. In AlbumDetailViewModel.RefreshFromLibrary (then lines 364-371), every LibraryUpdated ran Album = updatedAlbum. Every index rebuild creates a new Album (LibraryService.cs:1900), so OnAlbumChanged fired and called BuildRelatedSections. The method then always ran Tracks.ReplaceAll (a Reset) and BuildDiscGroups (DiscGroups.Clear), and called BuildRelatedSections a second time. The result: ev… | I stashed the source fix and ran the new class against the old code. IndexRebuild_WithTheSameTracks_KeepsTheRows failed: Tracks got 1 Reset where 0 was expected. The other two tests passed on the old code; they guard the new skip logic (in-place disc regroup, rebinding of fresh instances). With the fix applied, the warncheck build passed. I then ran 'dotnet… Tests added: 3. |
| U07 | medium | fixed | `6b8883e4` | Confirmed at HEAD 535f2ed8. The ArtistDetailViewModel constructor subscribed `(_, _) => Dispatcher.UIThread.Post(Rebuild)` (ArtistDetailViewModel.cs:339). Nothing checked visibility or IsPublishingPartial. Classify (:351-375) called ContainsArtistToken for every album and track, and ContainsArtistToken (LibraryAlbumsViewModel.cs:1298-1299) ran the regex split on both the credit and the artist name for every track th… | With the old handler temporarily restored (`Post(Rebuild)`, no partial or IsActive check), HiddenPage_DefersLibraryRebuild_UntilShownAgain and PartialScanPublish_KeepsTheLists_AuthoritativePublishRebuilds both FAILED (2 failed / 11 passed in ArtistDetailRebuildGuardTests). With the fix restored and rebuilt, `dotnet test --no-build --filter` over ArtistDetai… Tests added: 3. |
| U08 | medium | fixed | `64f74d4b` | Confirmed at HEAD 6b362c83. The BulkLyricsViewModel constructor (BulkLyricsViewModel.cs:44-45) created one Row per selected track. Each Row (:113) read Track.SyncedLyrics/Lyrics. Those getters (Track.cs:250-257) call LyricsStore.Read, which does a synchronous File.Exists/ReadAllText/deserialize and keeps only a 16-entry LRU (LyricsStore.cs:52, 126-143). All of this ran on the UI thread from MetadataHelper.OpenBulkLy… | warncheck.sh: build exit=0, no new warnings. The targeted tests passed 20/0 (BulkLyricsRowStatusTests, LyricsStudioDraftStoreTests, LyricsStudioStartOrderTests, LyricsStudioElrcRowTests), and 7/0 again after the final rebuild. Fail-without-fix check: I temporarily put the synchronous status back in the Row constructor. Ctor_LeavesLyricsStateToTheBackground … Tests added: 3. |
| U09 | medium | fixed | `c973f5b5` | Confirmed at HEAD 4c3a1690. In both pickers, `partial void OnSearchTextChanged(string value) => RefreshResults();` ran on every keystroke: LyricsBackgroundPickerViewModel.cs:127 and LyricsStudioPickerViewModel.cs:104. Each run scanned Albums and Tracks on the UI thread using SearchText.Matches and PlaylistViewModel.MatchesSearch. Both of those re-normalize the query and every field on each call (SearchText.cs:54-56)… | Ran warncheck.sh: build exit=0. To confirm the new tests catch the defect, I temporarily made both OnSearchTextChanged handlers call RefreshResults() directly again, rebuilt, and ran the two new tests. Both failed with "Assert.Empty() Failure: Collection was not empty". I then restored the fix, rebuilt, and ran dotnet test --no-build with the filter LyricsS… Tests added: 4. |
| U11 | medium | fixed | `10122282` | Confirmed at HEAD: MetadataViewModel.cs:2622-2644 (pre-fix) ran the multi-select rename loop (ComputeRenamedPath File.Exists, SuppressWatcherForRename, File.Move, MoveLyricSidecars) inline after the `await Task.Run` tag writes, which resume on the UI context. Also swallowed failures silently. Fix: snapshot _albumTracks and RenamePattern, run the loop in Task.Run with the same order for each file (so conflict detecti… | The new test runs Save on a UI-like SynchronizationContext and sets App.Services to a recording ILibraryWatcherService. SuppressPaths is called right before each move, so it shows which context the moves run on. The test sits in a new non-parallel collection "App.Services global" and restores App.Services afterwards. With the source change reverted, the tes… Tests added: 1. |
| U13 | medium | fixed | `306b0870` | Confirmed at HEAD in PlayerViewModel.cs, inside OnLibraryUpdated (around line 3362 at HEAD). Tracks deleted from Up Next and History were each removed with Collection.Remove on the UI thread. Every Remove did a linear search and shift and raised a CollectionChanged. MainWindow.axaml.cs:517 answered each event with QueueRowSelection.Apply and a posted renumber. The mini player, CoverFlow, LocalApi and waveform handle… | Before the fix, the new test failed: 'Expected: 1, Actual: 3' Up Next CollectionChanged events. With the fix it passes. It checks order is kept, the external track survives, History is pruned, there is exactly 1 event per list, and there are 0 events when nothing was removed. warncheck.sh: build exit=0 and no new warnings. Targeted tests passed 55/0 (filter… Tests added: 1. |
| U14 | medium | fixed | `013e9b46` | Confirmed at HEAD 25180009: SendToFolderViewModel.cs:54 `OnDestinationChanged => RebuildPlan()` ran RebuildPlan (61-88) synchronously on the UI thread. That covered Directory.Exists plus SendToFolderPlanner.Plan, which runs DiskProbe FileInfo stats on each track's target, source and .lrc. SendToFolderDialog.axaml:80 binds TextBox Text={Binding Destination} with the default PropertyChanged trigger, so this happened o… | Both new [AvaloniaFact] tests failed against the old view model. For that run, the HEAD version of SendToFolderViewModel.cs was restored temporarily with only a stub `PlanRebuild => Task.CompletedTask` added. The failures were "Assert.Empty() Failure: Collection was not empty" (the plan was built synchronously during typing) and "Assert.False() Failure" (Co… Tests added: 2. |
| U17 | medium | fixed | `a46d98ac` | Re-verified at HEAD 49d59853: StatisticsViewModel.Refresh() (line 109) ran ComputeOverview, ComputeQuality and ComputeHistory inline on the UI thread. Navigate("statistics") reaches it through MainWindowViewModel.cs:1955 and RefreshAndReturnStatistics at :2102, and Settings > View All Stats reaches the same path through :522-531. The fix follows SettingsViewModel.RefreshLibraryStatsAsync. On the UI thread, Refresh()… | warncheck.sh: build exit=0, no new warnings. dotnet test --no-build with the filter StatisticsRefreshOffUiThreadTests/StatisticsBarWidthTests/ListeningStatsCalculatorTests: 15 passed, 0 failed. Negative checks, done temporarily and then restored (cmp confirmed the committed file matches the fixed version): (1) with Compute run inline on the UI thread, which… Tests added: 2. |
| U18 | medium | fixed | `00070307` | Confirmed at HEAD 3ad7a8d8. Five dialog lists built every row as soon as the dialog opened, even with virtualization expected: - AudioConverterDialog.axaml:165 used the default panel. - BulkLyricsDialog.axaml:64-66 used StackPanel Spacing=4. - MetadataFinderDialog.axaml:108 used the default panel. - DuplicateFinderDialog.axaml:111 used the default panel. - SendToFolderDialog.axaml:109-111 used StackPanel Spacing=4. … | Each test opens the real dialog headless at 1000x800 with 400 rows and counts ItemsControl.GetRealizedContainers(). A test passes when more than 0 and fewer than 60 rows are realized. Before the fix, all 5 tests failed with 400 of 400 rows realized. After the fix, all 5 pass (`dotnet test --no-build --filter FullyQualifiedName~ToolDialogVirtualizationTests`… Tests added: 5. |
| U21 | medium | fixed | `32fee30c`, `41fd975e` | Confirmed at HEAD 816aa8ee. The scroll-to-top handlers in LibrarySongsView.axaml.cs:225, LibraryAlbumsView.axaml.cs:175 and LibraryArtistsView.axaml.cs:112 ran on every CollectionChanged while a filter was active, and every ReplaceAll raises one. That includes each LibraryUpdated Refresh (Songs VM:89-95, 517), the Albums column-count rebuild on resize, and the Artists portrait-refresh debounce and favorite toggle. T… | Headless Avalonia tests mount the real views in a Window, apply a search, scroll to 1500 px, raise LibraryUpdated and assert the offset is kept. The leave-and-return case detaches and re-attaches the Songs view, checks the offset is restored, applies a new search and asserts offset 0. I reverted only the view files (FilterKey left in the VMs) and 4 of the 5… Tests added: 5. |
| U22 | medium | fixed | `d2e2c89e` | Confirmed at HEAD. The side panel is built once in EnsureLyricsPanelLoaded (MainWindow.axaml.cs:720) and stays in LyricsPanelHost. Closing it only sets the wrapper's Width to 0 and then IsVisible=false (MainWindow.axaml.cs:550-556 before the fix). IsVisible=false does not detach the view, so DetachedFromVisualTree never fired. That handler (LyricsPanelView.axaml.cs:87-91) was the only place that called SetLyricsSurf… | The new headless test puts LyricsPanelView inside a Border wrapper, the same layout MainWindow uses, and turns the flowing backdrop on. It checks that the panel is counted once (_visibleLyricsSurfaces read by reflection) and that the flow is running. It then hides the wrapper and calls SetShown(false) and checks that the view is still attached, the count is… Tests added: 1. |
| U24 | medium | fixed | `c100ae76` | Confirmed at HEAD 75a09352. MiniPlayerWindow.axaml.cs:201 creates CoverSpinner(this, PillCoverSpin). OnClosed (1137-1161) stopped the other timers and unhooked Player.PropertyChanged, which is the only path to UpdatePillSpin, but it never stopped _pillSpinner. CoverSpinner.OnFrame (CoverSpinner.cs:57-74) re-queues on TopLevel.RequestAnimationFrame for as long as the clock is not settled, so a spinner left running ke… | The new headless test runs the mini player in the Pill design with Player.State=Playing and checks that the cover is turning. It then closes the window, pumps past the 170ms close fade, records the RotateTransform angle, pumps another 400ms and asserts the angle has not changed. Before the fix it FAILED (Expected 24.87, Actual 44.73: the closed window's cov… Tests added: 1. |
| U26 | medium | fixed | `66fbe35c` | I re-checked the defect at HEAD d4cd2500 and it was still there. OrganizeFilesDialog.axaml:150 put an ItemsControl on the default StackPanel with no ItemsPanel. OrganizeFilesViewModel.cs:29 declared Rows as a plain ObservableCollection. OrganizeFilesViewModel.cs:75-77 filled it with Rows.Clear() and then one Rows.Add per plan entry on the UI thread. MetadataHelper.cs:51 passes every local library track to the tool. … | I checked that the new test fails without each half of the fix. With both source files reverted, the test failed at the change-count assertion, because Update Preview raised more than one CollectionChanged event. With only the XAML reverted, it failed with '400 of 400 rows are realized: the list is not virtualizing'. With the fix in place, all 6 tests pass:… Tests added: 1. |
| X02 | medium | fixed | `42d79aba`, `c69f9d9b` | Confirmed at HEAD: NoctisServer.cs:122-129 keyed the lockout on ctx.Connection.RemoteIpAddress and checked it before Authenticate. Behind a reverse proxy every client shares one address, so 8 bad logins, or one device using token auth (error 41 also counts as a failure at :134), locked out every account and every API key for 15 minutes. Failures and the lockout are now keyed on remote address plus account name, with… | Checked that both new tests fail without the fix. I reverted NoctisServer.cs and removed the prune call, then rebuilt: both new tests failed (2 failed / 0 passed). With the fix restored: dotnet test --no-build --filter "FullyQualifiedName~LoginThrottleTests/FullyQualifiedName~NoctisServerTests" gives 17 passed, 0 failed. The existing RepeatedBadLogins_LockT… Tests added: 2. |
| X03 | medium | fixed | `a6ea03c6` | Confirmed at HEAD de64164a. At NoctisServer.cs:227-228, the download branch set ctx.Response.Headers.ContentDisposition to the raw Path.GetFileName(track.FilePath). ConfigureKestrel (NoctisServer.cs:66-74) sets no ResponseHeaderEncodingSelector. So Kestrel threw on any non-ASCII name, and the catch-all turned that into a Subsonic 'Server error'. The fix is the proposed one: the download branch now passes the name to… | The new test runs against real Kestrel on a loopback port. It downloads "Beyoncé - 夜.mp3" through /rest/download.view and asserts a 200 response, the exact file bytes, DispositionType 'attachment' and FileNameStar equal to the original name. Before the fix it FAILED: the body was an XML error envelope ([60,63,120,...] = "<?x") instead of the bytes [1,2,3,4]… Tests added: 1. |
| X05 | medium | fixed | `a429a1c9` | The finding was still present at HEAD 5fb54f9b. In EnhancedLrcParser.cs, ParseLine (lines 77-121) had no cap on words per line. MergeSyllables (lines 182-192) rebuilt `prev.Text + cur.Text` on every joinable syllable, which is O(n²). AppendBackground (lines 282-286) copied the whole background row on each call, also O(n²). In LyricsViewModel.cs, the `[bg:]` branch (lines 2763-2769) hit `continue` before any line was… | Before the fix (only the constant added so the tests compile), 4 of the new tests failed. AppendBackground_StopsGrowingPastWordCap failed on count. ParseLine_MoreWordsThanCap failed because words were not null. BgLines_CountTowardLineCap failed with 2 lines instead of 1, and took 4 s. MergeSyllables_LongJoinableRun failed at 8.38 s for 200k syllables agains… Tests added: 6. |
| X06 | medium | fixed | `1d7cf10f` | Confirmed at HEAD 6a7ea625. LogRedaction.cs:30-31 removes a query string only when it follows "http(s)://". BareSecretRegex (:20) matches only api_key, apikey, access_token and token. Subsonic URLs carry u, t and s, or p=enc:<hex> (SubsonicClient.cs:255-268). The VLC bridge (VlcAudioPlayer.cs:5175, 5198-5206) passes every line through DebugLog.Write, which only calls Scrub (DebugLog.cs:55). The demux "location='host… | Before the fix, the new theory failed 2 of 9 tests; for example, the actual output kept "?u=demo&p=enc:73656372657431". After the fix, `dotnet test tests/Noctis.Tests/Noctis.Tests.csproj --no-build --filter "FullyQualifiedName~LogRedactionTests"` passed 9 of 9, and the existing tests are unchanged and still pass. I did not check this at runtime against real… Tests added: 1. |
| X09 | medium | fixed | `267ac8ec`, `fff86f35` | Confirmed at HEAD 276dde1d. The MaxLyricLines=3000 cap was only enforced in LyricsViewModel.SplitPlainLyrics and ParseLrcContent. LyricsfileParser.Parse (src/Noctis/Services/LyricsfileParser.cs:54-117) and TtmlParser.Parse (src/Noctis.Core/Services/TtmlParser.cs:78-85) added every line with no count check, and the same was true for words per line, which the X05 fix had capped only for LRC. Their output reaches the n… | I temporarily restored LyricsfileParser.cs and TtmlParser.cs to their HEAD versions, keeping the shared constant so the tests compile, rebuilt, and ran the TtmlParserTests and LyricsfileParserTests classes. Result: 44 passed, 4 failed. The 4 failures are the new over-cap tests: both line-cap cases, including the YAML alias case, and both word-cap cases. I t… Tests added: 6. |
| X11 | medium | fixed | `c3b4562f` | Confirmed at HEAD in src/Noctis/Services/Plugins/ContentPack.cs:395-412. PlaceholdersFit/Formats validated pack values with the string "x", which ignores format specifiers, so "{0:D999999999}" passed. Loc.T then formats that text with an int, for example ContentPack.cs:162 Summary -> Loc.T("Plugins.Content.Themes", Themes.Count), and asks for about a billion digits. Alignment was also unbounded below .NET's 1,000,00… | I ran the new theory without the fix, with ContentPack.cs temporarily reverted and the test project rebuilt. It failed 3 of 8 cases: {0:D999999999}, {0,999999}, and {0:D999999999} against an English {0:N0}. With the fix restored and the project rebuilt, `dotnet test ... --filter "FullyQualifiedName~ContentPack"` passed 69 of 69, 0 failed. I did not run the … Tests added: 1. |
| X12 | medium | fixed | `5629bc7d` | Re-verified at HEAD 14a83660. PluginHost.cs:695-704 (MigrateSettings) reads CommunityPluginsEnabled == null as an install that predates the switch. When any code plugin is on disk, it turns community plugins on and grants every non-disabled plugin all of its declared permissions. Two paths set the value back to null while the plugins folder stays on disk: - ConfirmResetLibrary saved a bare `new AppSettings()` (Setti… | With the source fix stashed and the tests rebuilt: - DefaultSettings_DecideTheCommunityPluginsSwitch_AsRestricted FAILED. - UnrecoverableSettings_LeaveInstalledPluginsRestricted_AndUnapproved FAILED: a corrupt settings.json with no backup, plus the sample DLL plugin, loaded as Running with its permissions granted. - The legacy guard SettingsFromBeforeTheCom… Tests added: 3. |
| A10 | low | fixed (by A07's commit) | `dbfcf7a1` | Already fixed at HEAD (795c45e4) by the earlier lane commit 1071eddb "Fix A07: park the engine provider on pause instead of pausing WasapiOut". A10 depended on pause being WasapiOut.Pause(), which stopped the provider reads, so _silentSamples stayed 0 and _lastFrame kept the stale pre-pause frame. At HEAD, GaplessSink.Pause only sets Provider.Parked = true (src/Noctis/Services/GaplessSink.cs:387) and the stream keep… | I wrote a temporary probe class, tests/Noctis.Tests/TmpA10ProbeTests.cs, and deleted it afterwards without committing it. It modelled the A10 scenarios against HEAD's GaplessSpliceProvider: (1) park and render, then Clear + enqueue a new track, un-park, gate hold, then audio; (2) park, seek Flush while paused, un-park, refill; (3) park, Clear + new track re… |

### 2.5 Silent runtime re-verification on the merged branch

This is the same harness, test tones and actions as 2.2, run against the final `audit/full-pass` build (with the Phase 3 package updates). Both silence gates passed on each run, and loopcap showed **no audio session** for Noctis.

| Check | v1.5.4 + logging (2.3) | Merged `audit/full-pass` |
|---|---|---|
| Seek clicks (R3) | 3 of 4 seeks spliced with no ramp | **0 of 4.** The only jump left in 50.6 s of engine output is the natural end of track B (a 4-sample ramp to 0 before the MP3's encoder-delay silence), which is the same content as the baseline. |
| Pause misread as a stall (R1) | `RenderStall 2030 ms`, `PtsGap 2005`, `ReadAheadRaised 1000->3500` | **None.** The log shows only `Playback.Pause | engine=True … vlcState=Playing` / `Playback.Resume | engine=True … inputEnded=False`. |
| Memory trim during playback (R2) | forced gen2 GC, `RenderStall gapMs=39.1, gcPauseMs=34.1, gcs=1/1/1` | `[Memory] trim after startup scan: managed 50 MB, background collection (audio playing)`. One ordinary gen1 GC still showed up as `RenderStall gapMs=32.1, gcPauseMs=24.0, gcs=1/1/0`; the 100 ms buffer absorbs it (see "Not fixed"). |
| Restore seek (A26) | `Seek.Dropped reason=noMedia` → `PlayTrack seekMs=31991` | **`Seek.Deferred | reason=stopped, targetMs=15000`** → **`PlayTrack … seekMs=15000, seekSource=restore`**. The player-lane run measured 15.0 s at Play and 17.9 s three seconds later. |
| Replayed audio (buzz signature) | 0 | 0 |
| Natural gapless boundary | `Spliced … crossfadeMs=0` | `Spliced … crossfadeMs=0` |
| UI-thread stalls | 437 ms (startup), 113 ms (first play) | 354 ms and 159 ms, both during startup, before the first play. Not targeted by any finding; see "Not fixed". |

## Phase 3 — Dependencies

**Method:** `dotnet list <project> package --vulnerable --include-transitive` and `--outdated` (plus `--highest-minor`) were run for Noctis, Noctis.Core, Noctis.UI, Noctis.Core.Server, Noctis.Server, Noctis.Plugins.Abstractions, the Kawarp plugin and the tests on 2026-09-25 against nuget.org. Release notes, commit ranges and advisories were then researched for every candidate (web, cited below). Each update was applied as its own commit, followed by a build with the count-aware warning check and the full test suite.

**Known vulnerabilities: none.** No project has a vulnerable package, direct or transitive. The SDK in use (10.0.401) already ships runtime 10.0.12, the latest .NET 10 servicing release (Sep 8, 2026), including CVE-2026-69439, -71328, -69522, -69304, -58649 and -69806.

### Patch/minor updates

| Package | Current → latest (same major) | Decision | Evidence |
|---|---|---|---|
| Avalonia, .Desktop, .HarfBuzz, .Themes.Fluent, .Fonts.Inter, .Skia (plugin), .Headless.XUnit (tests), .Android (Android) | 12.1.2 → **12.1.3** | **Updated** (`574626a`) | Fix-only release ([notes](https://github.com/AvaloniaUI/Avalonia/releases/tag/12.1.3)): a theme-switch crash (#22189), X11 hide/UnmapNotify (#22135) and HiDPI/session-close fixes, case-insensitive font names, a SplineEasing constructor fix (#22169), and headless popups and disposal fixes. No security items. All Avalonia packages moved together (Android/Mobile included), so the shared Noctis.UI never mixes versions. Zero new warnings; full suite 3243/0/2. **Needs testing on Linux:** the suspend/resume window remap hides and shows the main window, and #22135 changes what X11 sees on hide. |
| LibVLCSharp | 3.10.0 → **3.10.1** | **Updated** (`8c2e90e`) | The 16 commits are all Uno/WinUI, UWP, Android and MAUI ([NEWS](https://github.com/videolan/libvlcsharp/blob/3.x/NEWS)). The desktop core looks untouched going by the commit titles; the file diff would not render, so this is UNVERIFIED. VLC, gapless and player tests: 272/272. |
| Microsoft.Data.Sqlite | 8.0.29 → **8.0.31** | **Updated** (`cd48e72`) | Infrastructure-only servicing. The only Sqlite change is the minimum SQLitePCLRaw bump to 2.1.12 ([8.0.31](https://github.com/dotnet/efcore/releases/tag/v8.0.31)). |
| SQLitePCLRaw.bundle_e_sqlite3 | 2.1.12 → **2.1.13** | **Updated** (`cd48e72`) | One commit fixing a net461 targets file ([compare](https://github.com/ericsink/SQLitePCL.raw/compare/v2.1.12...maint_21)). Same native SQLite 3.53.3, with no CVEs between the two (CVE-2025-6965/-7709/-70873 and CVE-2026-11822/-11824 are all fixed at or below 3.53.3; [sqlite.org/cves](https://www.sqlite.org/cves.html)). SQLite/sync/index/server tests: 96/96. |
| Net.Codecrete.QrCodeGenerator | 3.1.0 → **3.2.1** | **Updated** (`26b9da3`) | Speed/size improvements with the same API for the `EncodeText`/`Size`/`GetModule` calls Noctis makes ([releases](https://github.com/manuelbl/QrCodeGenerator/releases)). The QR for the same URL may now be smaller, and it still scans (WebRemoteQrTests pass). |
| Tmds.DBus.Protocol | 0.94.1 → **0.95.1** | **Updated** (`26b9da3`) | Its only Protocol change fixes MatchRule namespace keys (#454) that MPRIS and login1 do not use ([compare](https://github.com/tmds/Tmds.DBus/compare/rel/0.94.1...rel/0.95.1)). Avalonia.FreeDesktop requires >= 0.94.1. |
| Microsoft.NET.Test.Sdk (tests only) | 18.8.1 → **18.10.1** | **Updated** (`26b9da3`) | Drops the Mono fallback and surfaces test-host crashes ([18.10.0](https://github.com/microsoft/vstest/releases/tag/v18.10.0)). The MTP host is off by default, so the xunit v3 path is unchanged. |
| **NAudio.Core / NAudio.Wasapi** | 2.3.0 → 2.4.0 | **HELD — breaks the build** | `BufferedWaveProvider.BufferDuration` lost its setter (verified in the 2.4.0 binaries), and `src/Noctis/Services/WasapiGainOutput.cs:238` sets it. 2.4.0 also changes WasapiOut's stop and end-of-stream logic (#1246, #1248; [compare](https://github.com/naudio/NAudio/compare/v2.3.0...v2.4.0)). `Pause()`, latency handling, the Console role, WaveBuffer, SampleToWaveProvider and MMDeviceEnumerator are unchanged. Nothing in it helps Noctis, and no security fixes. If it is ever taken: construct the provider with `new BufferedWaveProvider(format, TimeSpan.FromMilliseconds(1000))` and re-test gapless, pause/resume, device unplug and exclusive mode by ear. |
| **VideoLAN.LibVLC.Windows** (native libvlc) | 3.0.23.1 → 3.0.24 | **HELD — flagged for you** | VLC 3.0.24 ([NEWS](https://raw.githubusercontent.com/videolan/vlc/3.0.x/NEWS), [release](https://www.videolan.org/vlc/releases/3.0.24.html)): **FFmpeg upgraded 4.4 → 8.1**, mmdevice crash/leak/deadlock fixes, MKV/MP4/TS/Ogg demux crash fixes, and many OOB/overflow/use-after-free fixes. Noctis forces `--demux=avformat` and feeds the gapless engine through amem, so a four-major FFmpeg jump can move start-of-track timestamps, seek landing points and gapless timing, none of which a build or unit test can catch. macOS CI bundles VLC 3.0.23, so bumping only Windows would split the platforms. **Recommendation:** within 1–2 weeks, bump Windows (NuGet) and macOS (`.github/workflows/dotnet.yml:213-214`) together and do a listening pass on gapless, seek, mmdevice and a range of formats. The security fixes argue for not waiting long, because Noctis decodes untrusted media. |

### Major-version updates (not applied — for you to schedule)

| Package | Latest | What would break in Noctis | Recommendation |
|---|---|---|---|
| Microsoft.Extensions.DependencyInjection | 8.0.1 → 10.0.12 | Nothing. The build already uses the 10.0 copy through `FrameworkReference Microsoft.AspNetCore.App` (`Noctis.csproj:54`); that is why **NU1510** fires ([docs](https://learn.microsoft.com/en-us/nuget/reference/errors-and-warnings/nu1510)). | **Delete** the PackageReference (`Noctis.csproj:88`) rather than upgrading it. This also removes the NU1510 warning. |
| Microsoft.Data.Sqlite | 8 → 10.0.12 | Probably nothing. v10 changes `GetDateTime`/`GetDateTimeOffset` UTC handling ([EF10 breaking changes](https://learn.microsoft.com/en-us/ef/core/what-is-new/ef-core-10.0/breaking-changes)), but Noctis stores dates as ISO text and parses them itself. | Safe to bump when convenient (8.x support ends Nov 2026). Run the SQLite/sync tests. |
| System.Security.Cryptography.ProtectedData | 8.0.0 → 10.0.12 | Nothing expected. Only new span overloads; the `Protect/Unprotect(byte[], null, scope)` calls stay unambiguous (`PersistenceService.cs:435, 455`). | Safe to bump. The duplicate reference at `Noctis.csproj:132` can go. |
| SQLitePCLRaw.bundle_e_sqlite3 | 2 → 3.0.5 | Packaging only, but only Microsoft.Data.Sqlite **11** is built against 3.x. Pairing with 8/10 is UNVERIFIED. | Wait; bump together with Microsoft.Data.Sqlite 11. |
| SkiaSharp | 3.119.4 → 4.152.1 | `SKFilterQuality` is removed (13 uses in 7 files), and the SKPaint text API is gone (ShareCardRenderer: about 13 `DrawText`, 6 `MeasureText`, font properties). Avalonia 12.1.x is compiled against SkiaSharp 3; a runtime crash with SkiaSharp 4 is reported ([SkiaSharp#3865](https://github.com/mono/SkiaSharp/issues/3865)). | **Blocked** until Avalonia 12.2 ([Avalonia#21696](https://github.com/AvaloniaUI/Avalonia/issues/21696)); bump both together along with the Kawarp plugin. |
| NAudio | 2 → 3.1.0 | Not researched in depth. 2.4.0 already breaks one call, and 3.x is a major redesign. | Only with a dedicated audio pass. |
| YamlDotNet | 15.3.0 → 18.1.0 | Probably nothing. The only use is LyricsfileParser with a plain `DeserializerBuilder` plus the new alias-refusing parser wrapper. Whether `IParser` changed is UNVERIFIED. | Low risk; bump and run the Lyricsfile tests. |
| xunit.v3 / xunit.runner.visualstudio | 3.2.2 / 3.1.5 → 4.0.1 / 4.0.0 | Test discovery fails with Avalonia.Headless.XUnit (built against xunit 3.2.2), affecting 595 `[AvaloniaFact]`/`[AvaloniaTheory]` tests. | **Blocked** until Avalonia.Headless.XUnit ships an xunit-4 build. |

**Also noted:** .NET 10.0.12 has a known GC crash on Windows machines with more than 64 CPU cores (fix due in 10.0.13; workaround `DOTNET_GCCpuGroup=1`). Rebuild with the next SDK once 10.0.13 ships.

## Final report

### Branches and what is on each

All branches are local and nothing is pushed. Each one is based on `3a2e13d` (v1.5.4). None of them has an AI trailer: the ones the agents added were stripped, and the rewritten branches have trees byte-identical to the originals.

| Branch (worktree) | Contents | State |
|---|---|---|
| **`audit/full-pass`** (`wt-audit`) | This report; silent test mode + logging; **77 audit fixes + 3 partial**; X16; GitHub **#70** and **#97**; Phase 3 package updates. 98 commits on top of v1.5.4. | Build exit 0, **zero new warnings**, full suite **3379 passed / 0 failed / 2 skipped** at `7363b45` (3375/0/2 twice before X16). |
| `feature/playlist-group-reorder` (`wt-playlist-groups`) | Luwi's request: drag playlist groups with the same liquid animation (`3213a3c`, `1e3fe8c`). | 15 new tests; full suite 3111/0/2; zero new warnings. **Not merged into the audit branch** (a separate feature). |
| `feature/music-video-audio` (`wt-music-video-audio`) | aaron's "Music video audio" setting, built to the picks you approved (4 commits, `357fbcd`…`330d345`). | 33 new tests; full suite 3129/0/2; zero new warnings; 2 reviews + fix-up. **Not merged.** |
| `feature/auto-update` (`wt-auto-update`) | Opt-in "Update automatically" in Settings → About (4 commits, `41bc01f`…`348e083`; the first is the same X16 fix that is on the audit branch). | Full suite 3218/0/2; zero new warnings; security + UX reviews + fix-up. **Not merged.** |
| `fix/issue-70-resume-cutout` / `fix/issue-97-startup-crash` | The GitHub fixes, also cherry-picked onto `audit/full-pass`. | — |

The feature branches do not include each other or the audit fixes. When you want one build with everything, merge them onto `audit/full-pass`. Expect conflicts in `PlayerViewModel.cs`, `VlcAudioPlayer.cs`, `SettingsViewModel.cs` and `SidebarViewModel.cs`.

### What was fixed (with commits)

- **Every High finding:**
  - A13 speed/pitch crash on the render thread (`81243bf`)
  - A24 stop-after-current ignored with gapless/AutoMix (`ad79a9c`)
  - P01 Intel-only ffmpeg in the Apple Silicon build (`741ac01`)
  - P25 distro libvlc on Linux tarballs (`382c253`)
  - S14/P27 rename-by-pattern losing play counts and favorites
  - S04 Organize Files playlist remap being overwritten
  - S07 right-click acting on the wrong selection, which could send files to the Recycle Bin
  - X04/P03 built-in server TLS broken on Windows
  - X07 "Move to Recycle Bin" permanently deleting on drives without one

  Full commit list: table 2.4.
- **All 68 Medium findings**, except 3 partial fixes (P04, A17, A18) described in "Found but not fixed".
- **The three known audio bugs:**
  - **Timeline slider desync on restore:** A26 (`Fix A26`), confirmed fixed at runtime (2.5).
  - **Clicks:** seek clicks R3 (`Fix R3`: VLC's stale resampler tail after a seek flush was being played, not a missing ramp; 0/4 seeks click now); pause/resume hard cut A07 (the engine provider is parked with a declick instead of pausing WasapiOut); mid-crossfade cut A09; the 44.1 kHz odd-sample declick A08.
  - **Buzz:** not reproduced at runtime (0 replayed windows), so there was nothing further to fix.
  - **Volume slider static:** the ramp fix is intact and nothing bypasses it. The related device-switch full-volume blip A05 was fixed. Whether the static is gone is **unverifiable silently** (see manual tests).
- **Runtime findings:** R1 (a pause no longer raises read-ahead), R2 (no forced gen2 GC while audio plays).
- **GitHub #70** (Linux: keep the output warm through long pauses, `89047f6`) and **#97** (diagnostics, `379cafa`, `6791eca`; the root cause is still unknown, see below).
- **Logging** (`185cd9b`, `bdcbfe3`) and a **silent test mode** `NOCTIS_AOUT=dummy` (`316bc35`).
- **Dependencies:** Avalonia 12.1.3, LibVLCSharp 3.10.1, Sqlite 8.0.31, SQLitePCLRaw 2.1.13, QrCodeGenerator 3.2.1, Tmds.DBus.Protocol 0.95.1, Test SDK 18.10.1 (`574626a`, `8c2e90e`, `cd48e72`, `26b9da3`).

### Found but NOT fixed, and why

1. **Partial fixes:**
   - **P04:** a case-only rename on Linux no longer strands the track. Two distinct files whose names differ only by case still share one ID, and one is dropped. Fixing that needs an ID-scheme change, which is a data migration.
   - **A17:** remote (media-server) gapless/crossfade now works on the Windows engine. The non-engine paths (macOS/Linux, exclusive mode) still cold-open remote streams. Not tested against a real server.
   - **A18:** ReplayGain **attenuation** is now sample-exact at the seam. **Boosts** still ride the session volume with the old early timing, because whether the Windows float mixer clips samples above 1.0 is unknown.
2. **57 Low findings** are outside the requested Critical/High/Medium scope and are listed in the table below. X15 is worth doing next: yt-dlp is auto-downloaded and executed with no checksum or signature check.
3. **Held updates:**
   - **NAudio 2.4.0:** it breaks the build at `WasapiGainOutput.cs:238` and gives Noctis nothing.
   - **libvlc 3.0.24:** a four-major FFmpeg jump under the gapless engine. Bump Windows and macOS together with a listening pass. All major versions are in the Phase 3 table.
4. **Runtime observations no finding covered:**
   - Startup UI-thread stalls of 350–440 ms.
   - `SessionVolume.Resolve` logged hundreds of times a minute while no session exists. Harmless when a session exists, but it would flood the log in a real "session lost" state.
   - Ordinary gen1 GC pauses (about 24 ms) still stall the render thread briefly. The 100 ms buffer absorbs them.
5. **Reviewer "low" notes left as they are:**
   - A26: seek-to-0 on a stopped RememberPlaybackPosition track resumes from the saved spot.
   - A19: `OnEndReachedCore` clears `_isPaused`, a narrow race.
   - P16: on macOS/Linux a runtime crash now shows the Windows-style blocking dialog.
   - P29: the tray-usable probe needs about 100 ms of D-Bus before close-to-tray works.
   - R3: the fixed 20 ms resampler-tail discard also applies to 48 kHz sources, which don't need it.
   - X07×P17: on NAS/USB volumes Lyrics Studio can no longer replace a foreign `.lrc`, because trash is refused there.
   - P08: more directory probes per cover lookup on Linux.
   - U06/U08/U14: minor.
6. **#97 root cause:** unknown. The branch adds breadcrumbs so the next crash log names the step and file. A reply to the reporter asking for stderr, coredumpctl and the kernel OOM log is drafted and **not posted**.
7. **RequestBringIntoView:** no such fix exists in v1.5.4 (see "Corrections to the brief"). Please say which fix you meant.

### Needs manual testing by you

**Windows (by ear, with real output — the silent mode cannot test these):**
1. **Seek:** drag the timeline repeatedly in FLAC 44.1/48 kHz, MP3 and hi-res files. There should be no click. (R3/A08)
2. **Pause/resume** with "fade on pause" OFF: no click at either edge. (A07)
3. **Volume slider:** drag fast during playback, check for no static, and **switch output device mid-song**: no full-volume blip. (A05, the known "volume static" bug)
4. **Speed ≠ 1× and pitch ≠ 0:** audio must keep playing. (A13/A11/A12)
5. **Stop after current track** with gapless and with AutoMix: it must stop. (A24)
6. **Restore:** close while paused mid-song, reopen, drag the slider **before** Play, then Play: it starts where you dragged. (A26)
7. **Exclusive mode:** seek and stop with no hard click. (A23)
8. **Laptop idle sleep:** leave Noctis open and paused; the PC should now be able to sleep. (A03)
9. **Files:** right-click a row outside a Ctrl-selection and Remove it; only that row goes (S07). Organize Files, then check playlists (S04). Rename by pattern, then check play counts and favorites (S14). "Move to Recycle Bin" on a USB stick must ask before permanently deleting (X07).
10. **General UI after Avalonia 12.1.3:** theme switch, menus, lyrics page.
11. **Feature branches:** music video audio (checklist in the branch commits), playlist-group dragging (checklist in `3213a3c`), auto-update (test with a pre-release or a faked older version).

**macOS:**
- Apple Silicon build, ffmpeg features without Rosetta (P01).
- "Open With Noctis" / double-click an audio file (P15).
- A startup error shows a dialog (P16).
- The bundled libvlc is used when VLC.app is missing or broken (P24).
- A Dock click restores a tray-hidden window (P30).

**Linux:**
- **#70:** pause for more than 10 minutes, then resume, on Fedora/PipeWire AppImage. There should be no cutout.
- **#97:** ask the reporter for the drafted diagnostics.
- A distro libvlc tarball starts (P25).
- inotify on a big library doesn't freeze and hitting the limit doesn't storm (P05/P06).
- Folder covers with capitalised names (Cover.jpg) show up (P08).
- AppImage URL and trash tools work (P12).
- Suspend/resume with a dialog open (P28; also re-test after Avalonia 12.1.3's X11 unmap change).
- GNOME without a tray (P29).
- MPRIS shuffle from playerctl (A34).

### Things I am unsure about (UNVERIFIED)

- **Real-device audio:** the silent harness renders the real engine chain into a null device, so it proves the PCM. It cannot prove what the device and OS mixer do: session-volume static, the drain at a pause edge, or device switches.
- **A08** could not trigger here (48 kHz device). It was fixed from the code and unit tests only.
- **R4 follow-up:** the engine reviewer saw a fade-in window correlate 0.994 with pre-seek audio about 41 ms before a cut. With a near-periodic sweep that is probably coincidence, but a dedicated check for stale pre-seek audio after a flush on 48 kHz FLAC was not done.
- **macOS and Linux fixes** were checked by reading the code, plus headless tests where possible. None were run on those OSes.
- **LibVLCSharp 3.10.1:** "desktop core untouched" rests on the commit titles, because the file diff would not render.
- **The #70 diagnosis** assumes the reporter's "a while" means longer than about 10 minutes. The drafted check `NOCTIS_KEEPALIVE_IDLE_MS=20000` would confirm it on v1.5.4.

### How this audit ran (for reproducibility)

- Worktrees live under `C:\Users\okfer\Downloads\Noctis\wt-*`. Your own tree `Noctis\` (branch `android-phase2`, with its 13 uncommitted files) was never touched.
- The silent harness (`drive.py` with its two silence gates, `analyze_tap.py`), the test tones and every run log live in the session scratchpad, which is temporary. Copy it if you want to keep it.
- About 67 GB of old build output and published-release staging was deleted from the Noctis folder with your approval (the details are in the conversation).

#### Low findings not fixed (outside the requested scope)

| ID | Location | Finding |
|---|---|---|
| A02 | `src/Noctis/Services/GaplessSink.cs:113` | NOCTIS_ENGINE_TAP rebuilds the render chain as Tap(Provider), silently removing the mute gate and the beat and spectrum tap |
| A04 | `src/Noctis/Services/GaplessSink.cs:244` | GaplessSink reads the device format and watches the Multimedia role, but NAudio's WasapiOut opens the Console role |
| A06 | `src/Noctis/Services/GaplessSink.cs:287` | The sink rebuild starts the new output before publishing it and keeps _rebuilding set during the callback, so an immediate failure of the new stream is dropped and the engine stays silent (or rebuilds in a hot loop) |
| A14 | `src/Noctis/Services/VlcAudioPlayer.cs:1160` | The volume ramp worker keeps writing the session during pause and crossfade fades, which the setters assume are parked, so the two fight |
| A15 | `src/Noctis/Services/VlcAudioPlayer.cs:1684` | Output-mode rebuild resumes at _player.Time, which on the engine is ahead of the audible position by the ring depth, so enabling Exclusive Mode skips ahead |
| A20 | `src/Noctis/Services/VlcAudioPlayer.cs:4370` | The engine seek re-bases the segment from the unclamped target while the worker seeks to the end-guarded target, so the timeline and lyrics run up to 1 s ahead |
| A33 | `src/Noctis/Services/VlcAudioPlayer.cs:4400` | The UI thread calls native libvlc get_length on every position tick and in Seek(), which can block while a worker's _player.Stop() joins a stalled input thread |
| A35 | `src/Noctis/ViewModels/PlayerViewModel.cs:2001` | PlayTrack counts a play, records history and fires TrackStarted before the file is opened, so missing or unparsable files get phantom plays |
| A36 | `src/Noctis/ViewModels/PlayerViewModel.cs:1536` | The desktop never persists the pre-shuffle order, so turning Shuffle off after a restart leaves the queue scrambled |
| A37 | `src/Noctis/Services/VlcAudioPlayer.cs:4243` | A pause arriving while VLC is still Opening/Buffering a new track is ignored: audio starts under a Paused UI |
| A38 | `src/Noctis/Services/MprisService.cs:522` | MPRIS SetPosition ignores the TrackId, so a stale scrub from a widget seeks the next track |
| P02 | `.github/workflows/dotnet.yml:422` | AppImage desktop entry has no MimeType= and no %F, so "Open with Noctis" (which the app supports) never appears in Linux file managers |
| P07 | `src/Noctis.Core/Services/LibraryWatcherService.cs:392` | Linux: the watcher's "file still being written" check can't see writers and passes on the first sample, so half-written files are imported |
| P10 | `src/Noctis/Helpers/LibraryRemovalHelper.cs:290` | Linux: whole-folder trash compares the removed paths case-insensitively and can trash a different file whose name differs only by case |
| P11 | `src/Noctis/Helpers/PlatformHelper.cs:124` | Linux 'Show in folder': dbus-send is run without --print-reply, so it exits 0 even when no file manager answers and the xdg-open fallback never runs |
| P13 | `src/Noctis/Helpers/PlatformHelper.cs:386` | Linux "System" theme: gsettings color-scheme 'default' is treated as a definite Light answer, so the portal/KDE/gtk-theme probes added for L24 never run |
| P14 | `src/Noctis/Helpers/StartupHelper.cs:129` | macOS launch at login saves whatever bundle path the app is running from (App Translocation, a mounted DMG, Downloads) and keeps showing ON after that path is gone |
| P18 | `src/Noctis/Services/MacNowPlayingService.cs:139` | macOS Now Playing always publishes playback rate 1.0 and ignores speed changes, so the Control Center position drifts at 0.75×–2× |
| P19 | `src/Noctis/Services/MacNowPlayingService.cs:174` | macOS Now Playing never shows album art: MPMediaItemArtwork initWithImage: does not exist on macOS, so the probe always skips the art |
| P20 | `src/Noctis/Services/MprisService.cs:192` | MPRIS never sends PropertiesChanged for Shuffle or LoopStatus, so desktop widgets and playerctl show stale repeat/shuffle state |
| P21 | `src/Noctis/Services/MprisService.cs:375` | MPRIS Raise skips ShowFromTray: it can show the main window next to the Topmost mini player and doesn't restore ShowInTaskbar |
| P23 | `src/Noctis/Services/SendToFolderService.cs:72` | Send to Folder (flat mode) keeps the source filename as-is, so Linux/macOS names containing : ? * " < > / fail on FAT32/exFAT/NTFS targets |
| S01 | `src/Noctis.Core/Services/LibraryService.cs:2133` | Metadata-schema backfill saves a startup snapshot of settings.json after a multi-minute pass, reverting changes made meanwhile |
| S02 | `src/Noctis.Core/Services/PersistenceService.cs:119` | A corrupt or unreadable settings.json is never surfaced to the user (SettingsLoadFailed has no reader); any read exception counts as corruption |
| S03 | `src/Noctis.Core/Services/PersistenceService.cs:602` | settings.json replace (File.Move overwrite) fails whenever another handle has the file open, and the failure is dropped with no retry or log |
| S05 | `src/Noctis/ViewModels/CommandPaletteViewModel.cs:104` | Command palette 'Go to Settings' shows Settings as an inline page whose close (X) button does nothing, bypassing the modal |
| S06 | `src/Noctis/ViewModels/CommandPaletteViewModel.cs:282` | Command palette: pressing Enter right after typing runs the previous result list's top row |
| S08 | `src/Noctis/ViewModels/LibrarySongsViewModel.cs:383` | The track menu's 'Favorites' / 'Remove from Favorites' label follows the clicked row, but the command flips each selected track: on a mixed selection, 'Favorites' unfavorites the ones already favorited |
| S09 | `src/Noctis/ViewModels/LrcEditorViewModel.cs:194` | The LRC editor stamps and seeks against whatever song is playing, even after playback has moved on to the next song |
| S11 | `src/Noctis/ViewModels/LyricShareViewModel.cs:852` | Lyric share clip export cannot be cancelled; the still-card export ignores cancellation and a cancelled export leaves a truncated .mp4 |
| S12 | `src/Noctis/ViewModels/MetadataHelper.cs:231` | Volume Adjust and EQ changes made in the multi-select or album metadata editor are not applied to the song that is playing |
| S16 | `src/Noctis/ViewModels/SettingsViewModel.cs:384` | Release-list refresh is wired to the About tab, but the version manager now lives on the Advanced tab |
| S17 | `src/Noctis/ViewModels/SettingsViewModel.cs:3043` | Changing any player/lyrics setting snaps every scrolling marquee title back to its start |
| S18 | `src/Noctis/ViewModels/SettingsViewModel.cs:3973` | Four newer sliders save settings.json on every value tick instead of using the debounced save |
| S20 | `src/Noctis/ViewModels/SettingsViewModel.Features.cs:473` | SyncDeviceId is generated on load but wiped by the first save's merge and never persisted |
| S21 | `src/Noctis/ViewModels/SidebarViewModel.cs:611` | Deleting the playlist that is currently open (sidebar right-click, Delete) leaves its page showing; later edits there are silently lost |
| S26 | `src/Noctis/Views/LyricsBackgroundPickerDialog.axaml.cs:70` | Closing the Lyrics Background picker with Esc leaves a YouTube backdrop download running, which then silently applies the video |
| S29 | `src/Noctis/Views/MetadataWindow.axaml:490` | Metadata editor Cancel stays enabled while Save runs; it closes the window, the save still completes, and any write-failure message is lost |
| U01 | `src/Noctis.UI/Controls/GlassPanel.cs:618` | GlassBackdropOp.Render allocates a native SKRoundRect on every frosted repaint and never disposes it |
| U02 | `src/Noctis/Controls/SpectrumVisualizer.cs:145` | Lyrics-page SpectrumVisualizer starts a 250 ms polling DispatcherTimer while the LyricsView is pre-built but never shown (or detached), and it never stops |
| U04 | `src/Noctis/ViewModels/AddSongsDialogViewModel.cs:195` | Add Songs dialog 'Deselect all' is O(n²): List<Guid>.Remove per matched track |
| U10 | `src/Noctis/ViewModels/MetadataViewModel.cs:686` | Metadata editor constructor and Save read every selected track's lyrics from the disk-backed lyrics store (and sidecars) on the UI thread |
| U12 | `src/Noctis/ViewModels/PlayerViewModel.cs:1698` | Every track change probes the track's music folder 7+ times (music video + animated cover) on the UI thread, even with those features off |
| U15 | `src/Noctis/ViewModels/SettingsViewModel.cs:2049` | In Developer Mode, the Settings log pane is rebuilt (a full re-join of the 500-line session log) on every log burst, even while Settings is closed |
| U16 | `src/Noctis/ViewModels/SidebarViewModel.cs:879` | Edit Playlist copies the chosen cover synchronously on the UI thread with no error handling; a failed copy silently discards the whole edit |
| U19 | `src/Noctis/Views/LibraryFoldersView.axaml:93` | Folders view TreeView is not virtualized; expanding a root with many subfolders realizes every node |
| U20 | `src/Noctis/Views/LibraryPlaylistsView.axaml:28` | Playlists grid still realizes every tile (UniformGrid in a ScrollViewer), with a per-tile Height binding to its own Bounds |
| U23 | `src/Noctis/Views/MainWindow.axaml:1009` | Queue popup rows each build an inline ContextMenu whose icon eagerly decodes a 512x512 PNG per realized row |
| U25 | `src/Noctis/Views/MiniPlayerWindow.axaml.cs:1538` | Closing the mini player during a drawer slide leaves MiniPlayerViewModel.IsDrawerAnimating stuck true, so an in-flight search fill re-polls every 50 ms indefinitely |
| U27 | `src/Noctis/Views/PlaylistView.axaml.cs:115` | PlaylistView re-stamps every realized row once per CollectionChanged event; removing a multi-selection fires one event per track |
| U28 | `src/Noctis/Views/SettingsView.axaml:3892` | Settings 'Removed tracks' list is unbounded, non-virtualized and rebuilt item by item on every Settings open |
| X01 | `src/Noctis.Core.Server/Services/Server/LibraryServerAdapter.cs:117` | Server playlist endpoints load, modify and save playlists.json without a lock: concurrent create/update/delete/sync requests lose each other's changes |
| X08 | `src/Noctis/Services/Loon/LoonClient.cs:688` | Relay-supplied Loon chunk size is cast to int unchecked: a chunk_size of 2^32 gives a 0-byte chunk and an endless send loop that takes one of the 3 request slots and never releases it |
| X10 | `src/Noctis/Services/Plugins/ContentPack.cs:386` | A content pack, which installs with no approval and works in restricted mode, can replace the English (or current-language) text of the plugin consent/safety dialogs and destructive-action labels |
| X13 | `src/Noctis/Services/Plugins/PluginInstaller.cs:57` | A corrupt, encrypted or LZMA-compressed plugin .zip throws InvalidDataException that the install flow does not catch: install silently does nothing |
| X14 | `src/Noctis/Services/WrapArchiveService.cs:167` | A failed load of wrap_archive.json is followed by a Save that overwrites the permanent Wrap archive with only the years still in the live log |
| X15 | `src/Noctis/Services/YouTube/YtDlpTool.cs:126` | The yt-dlp executable is downloaded and silently auto-replaced with no checksum or signature check, then executed |
